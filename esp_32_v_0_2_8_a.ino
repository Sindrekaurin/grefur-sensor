...
#define INPUT_MODULE 1
#define OUTPUT_MODULE 0
#define SCREEN_MODULE 0 // Only available on ESP32.
...
/*
   Debug configuration
   -------------------
   Keep DEBUG 0 for production builds. Set DEBUG to 1 for broad diagnostics,
   or enable one channel when investigating a specific subsystem.
   ...
   Runtime syntax is also supported for simple statements:
     if (DEBUG) Serial.println("General debug details");
*/
#define DEBUG 0
#define DEBUG_SYSTEM DEBUG
#define DEBUG_ERROR_VISUALIZATION DEBUG
#define DEBUG_ACTUATOR_MANAGER DEBUG
#define DEBUG_MUX_FUNCTIONALITY DEBUG
#define DEBUG_MQTT_MANAGER DEBUG
#define DEBUG_NETWORK_CONNECTION DEBUG
#define DEBUG_EXPRESSIONS DEBUG
#define DEBUG_BINDING_ENGINE DEBUG
#define DEBUG_DISPLAY_MANAGER DEBUG
#define DEBUG_SENSOR_MANAGER DEBUG
#define DEBUG_MODULE_SENSOR DEBUG
#define DEBUG_MODULE_OUTPUT DEBUG
#define DEBUG_ONMESSAGE DEBUG
#define DEBUG_PUBLISH DEBUG

#define DEBUG_PRINT(channel, message) do { if (channel) Serial.println(message); } while (0)
#define DEBUG_PRINTF(channel, format, ...) do { if (channel) Serial.printf(format, ##__VA_ARGS__); } while (0)
...
bool backupRestoreCompleted = false;
bool backupRestoreSucceeded = false;
String backupRestoreMessage = "";
...
#else
#define ONBOARD_LED 2 // Fallback on ESP32
#endif
...
int validGpioPins[] = {0, 1, 2, 3, 4, 5, 21, 43, 44};
...
GrefurModuleMaster ModuleEngine;
...
/*void setupOTA() {
    ArduinoOTA.setHostname("xiao-sensor");  // Optional
    ArduinoOTA.setPassword("hemmelig123");  // Recommended
...
/**
   ...
   Runtime:
    When configured WiFi is available:
      Connect to the configured access point.
      Load persisted settings.
      Start publishing to the MQTT broker.
      Serve the settings interface on the assigned IP address.
    When configured WiFi is unavailable:
      Create a local access point.
      Load persisted settings when available.
      Serve the settings interface through the device access point.
      Allow WiFi/MQTT changes, then save and restart.
...
struct SensorConfig {
  ...
  union {
    ...
    struct {
      int    upPin;               // Digital pin for count-up pulses   (-1 = expression only)
      int    downPin;             // Digital pin for count-down pulses (-1 = none)
      int    resetPin;            // Digital pin for reset             (-1 = none)
      bool   persistent;          // true = save/restore across power cycles
      // 5 bytes remaining fit neatly in the reserved space
    } counter;
    struct {
      uint8_t i2cAddress;    // Connected module address
      uint16_t registerIndex; // Register to read (Ex: TEMP, HUMIDITY)
      int8_t  dataType;      // GREFUR_T_I16, F32, etc.
      char    scaleUnit[8];  // Unit for scaled value (f.eks. "°C", "bar")
      float   scale;         // Scale incoming values, for example 237 raw -> scale 0.1 -> 23.7.
      float   offset;        // Offset applied to the received value.
      uint16_t timeoutMs;     // Time without answer before set "offline"
      uint8_t  errorCount;    // Number of error readings in a row (Runtime only)
    } module;
...
enum SensorType : int8_t {
  ...
  SENSOR_ANALOG = 1,                // Analog voltage/current sensor
  SENSOR_DIGITAL = 2,                // Digital on/off sensor
  SENSOR_COUNTER = 3,               // Up/down counter with persistent storage,
  SENSOR_MODULE_VALUE = 4
};
...
struct CalculationConfig {
  ...
  union {
    ...
    struct {
      char sourceTag[48];         // Source expression or sensor name
      float lastPersistedValue;   // Last value written to EEPROM
      uint32_t lastUpdateTs;      // Timestamp of last change
      uint32_t persistDebounceMs; // Debounce before EEPROM write
      bool mqttRetain;            // Publish with MQTT retain flag
...
struct ActuatorConfig {
  ...
  union {
    ...
    struct {
      ...
      float voltageMin;    // Physical voltage
      float voltageMax;    // Physical voltage
    } analog;
    struct {
      uint8_t  i2cAddress;      // Connected module address
      uint16_t registerAddress; // Register to write to
      int8_t   dataType;        // GREFUR_T_I16, F32, etc.
      char     unit[8];         // Unit for the output (e.g. "%", "°C")
      float    scale;           // Multiplier (Applied: Raw = (Value - Offset) / Scale)
      float    offset;          // Offset
      uint16_t timeoutMs;       // Command timeout
      uint8_t  errorCount;      // Runtime error tracking
    } module;
  };
...
enum ActuatorType : int8_t {
  ...
  ACTUATOR_MODULE_VALUE = 2       // Grefur I2C Module output
};
...
// Constants for screen (settings of connected screen)
#define OLED_WIDTH        128
#define OLED_HEIGHT        64
#define OLED_RESET         -1
#define OLED_I2C_ADDR    0x3C
#define MAX_SCREEN_WIDGETS  8   // maximal widgets in one screen

#if SCREEN_MODULE
// Content types inside widget
enum WidgetType : int8_t {
  WIDGET_NONE       = -1,
  WIDGET_LABEL      =  0,   // Static text
  WIDGET_VALUE      =  1,   // Values bound to dynamic object
  WIDGET_UNIT       =  2,   // Unit from object
  WIDGET_BAR        =  3,   // Horizontal progress bar
  WIDGET_DIVIDER    =  4,   // Horizontal static line
  WIDGET_CLOCK      =  5,   // System time clock
};
...
// Single widget of custom design
struct ScreenWidget {
  int8_t   type;                    // WidgetType
  uint8_t  x;                       // Pixel x-coordinate
  uint8_t  y;                       // Pixel y-coordinate
  uint8_t  textSize;                // TextSize
  char     sensorName[24];          // Bound sensor name (empty = unbound)
  char     staticText[24];          // For WIDGET_LABEL
  float    barMin;                  // For WIDGET_BAR
  float    barMax;                  // For WIDGET_BAR
  uint8_t  barWidth;                // For WIDGET_BAR (piksler)
  uint8_t  decimals;                // Number of decimals for WIDGET_VALUE
};

// Screendesign stored in eeprom
struct ScreenDesign {
  uint8_t      magic;               // 0xD1 = valid design
  uint8_t      widgetCount;
  ScreenWidget widgets[MAX_SCREEN_WIDGETS];
  uint16_t     refreshMs;           // Refresh interval in milliseconds
  char         title[20];           // Optional heading
};
...
static_assert(sizeof(EEPROMLayout) <= EEPROM_SIZE, "EEPROMLayout is larger than EEPROM_SIZE!");
...
/* Middleware / Logic between the server and the client,
 *  */
    ...
    bool updateNumericField(T& dest, const String& newValue) {
      if (newValue.length() > 0) {
        ...
        if constexpr (std::is_same_v<T, int>) newVal = newValue.toInt();
          ...
          DEBUG_PRINT(DEBUG, "Given type is not supported");
          return false;
        }

        if (dest != newVal) {
          dest = newVal;
          DEBUG_PRINT(DEBUG, "Value updated");
          return (_changed = true);
        } else {
          DEBUG_PRINT(DEBUG, "Value is the same, no update needed");
        }
      } else {
        DEBUG_PRINT(DEBUG, "Given value has zero length");
      }
      return false;
    }
    ...
    bool updateIPAddressField(IPAddress& dest, const String& newValue) {
      ...
      for (int i = 0; i < 4; i++) {
        if (parts[i] < 0 || parts[i] > 255) {
          DEBUG_PRINT(DEBUG, "IP part out of range");
          return false;
        }
        newIP[i] = parts[i];
      }
...
class MqttManager {
  private:
    ...
    bool authenticate() {
      ...
      if (httpResponseCode == 200) {
        ...
        // The device no longer exists in the portal.
#if DEBUG_MQTT_MANAGER
  ...
  public:
    ...
    void clearBlockedSensor(const char* sensorName) {
      for (auto it = publishedMetadata.begin(); it != publishedMetadata.end(); ++it) {
        if (it->equals(sensorName)) {
          publishedMetadata.erase(it);
#if DEBUG_MQTT_MANAGER
          Serial.printf("[MQTT] Metadata block cleared for: %s\n", sensorName);
#endif
          return;
        }
      }
...
class SensorManager {
  ...
  public:
    ...
    float _currentReadings[MAX_SENSORS_ALLOWED] = {}; // Fresh measurements, not published.
    ...
    String retrieveSensorFieldsAsJson(int8_t type) {
      ...
      switch (type) {
        ...
        case SENSOR_COUNTER:
          ...
          fields["upPin"]     = "number";
          labels["upPin"]     = "Count up pin";
          placeholders["upPin"] = "e.g., 5";

          fields["downPin"]   = "number";
          labels["downPin"]   = "Count down pin";
          placeholders["downPin"] = "-1 = none";

          fields["resetPin"]  = "number";
          labels["resetPin"]  = "Reset pin";
          placeholders["resetPin"] = "-1 = none";

          fields["persistent"] = "boolean";
          labels["persistent"] = "Remember count after reboot";
          break;
        case SENSOR_MODULE_VALUE:
          doc["type"] = "Module";

          // I2C Hardware Addressing
          fields["i2cAddress"] = "number";
          labels["i2cAddress"] = "Module I2C Address";
          units["i2cAddress"] = "Hex/Dec";
          placeholders["i2cAddress"] = "e.g., 8 (0x08)";

          fields["registerIndex"] = "number";
          labels["registerIndex"] = "Register Address";
          placeholders["registerIndex"] = "e.g., 16 (0x10)";

          // Data Formatting
          fields["dataType"] = "number";
          labels["dataType"] = "Register Data Type";
          placeholders["dataType"] = "0:U8, 1:U16, 2:I16, 3:I32, 4:F32";

          // Scaling & Calibration
          fields["scale"] = "number";
          labels["scale"] = "Multiplier (Scale)";
          units["scale"] = "x";
          placeholders["scale"] = "0.1";

          fields["offset"] = "number";
          labels["offset"] = "Additive Offset";
          placeholders["offset"] = "0.0";

          fields["scaleUnit"] = "text";
          labels["scaleUnit"] = "Display Unit";
          placeholders["scaleUnit"] = "e.g., %RH";

          // Communication Watchdog
          fields["timeoutMs"] = "number";
          labels["timeoutMs"] = "Communication Timeout";
          units["timeoutMs"] = "ms";
          placeholders["timeoutMs"] = "1000";
          break;
          ...
          if (isConfigured) {
            switch (_config.sensors[i].type) {
              ...
              case SENSOR_DIGITAL:
                ...
                break;
              case SENSOR_COUNTER:
                sensor["details"] = "Counter (up=" + String(_config.sensors[i].counter.upPin) + ")";
                break;
              default:
    ...
    // Legg til som public metode i SensorManager:
    void initCounterReadings() {
      for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
        if (_config.sensors[i].type == SENSOR_COUNTER) {
          _currentReadings[i] = loadCountFromSensor(_config.sensors[i]);
        }
      }
    }
    ...
    */
    ...
    SensorUpdateResult updateSensor(int sensorId, const SensorConfig& updatedSensor) {
      SensorUpdateResult result;
      result.sensorId    = -1;
      result.unitChanged = false;
      memset(result.oldUnit, 0, sizeof(result.oldUnit));
      memset(result.newUnit, 0, sizeof(result.newUnit));
      ...
      // 2. Ensure the slot is already configured
      ...
      // ... Step 3. Capture old unit ...
      switch (_config.sensors[sensorId].type) {
        case SENSOR_NTC:          strncpy(result.oldUnit, _config.sensors[sensorId].ntc.unit,       sizeof(result.oldUnit) - 1); break;
        case SENSOR_ANALOG:       strncpy(result.oldUnit, _config.sensors[sensorId].analog.scaleUnit, sizeof(result.oldUnit) - 1); break;
        case SENSOR_MODULE_VALUE: strncpy(result.oldUnit, _config.sensors[sensorId].module.scaleUnit, sizeof(result.oldUnit) - 1); break;
        default: break;
      }

      // ... Step 4. Capture new unit ...
      switch (updatedSensor.type) {
        case SENSOR_NTC:          strncpy(result.newUnit, updatedSensor.ntc.unit,       sizeof(result.newUnit) - 1); break;
        case SENSOR_ANALOG:       strncpy(result.newUnit, updatedSensor.analog.scaleUnit, sizeof(result.newUnit) - 1); break;
        case SENSOR_MODULE_VALUE: strncpy(result.newUnit, updatedSensor.module.scaleUnit, sizeof(result.newUnit) - 1); break;
        default: break;
      }

      // 5. Detect unit change before overwriting
      result.unitChanged = (strncmp(result.oldUnit, result.newUnit, sizeof(result.oldUnit)) != 0);

      // 6. Perform full copy of new configuration
      ...
      if (DEBUG) {
        ...
        if (result.unitChanged) {
          Serial.printf("- Unit changed: '%s' -> '%s'\n", result.oldUnit, result.newUnit);
        }
      }
    ...
    /* Read sensor value based on configured type, including Grefur Modules */
    float readSensor(int sensorIndex) {
      ...
      switch (_config.sensors[sensorIndex].type) {
        ...
        case SENSOR_COUNTER:
          return readCounter(sensorIndex);
        case SENSOR_MODULE_VALUE:
          return readModule(sensorIndex);
        default:
          return NAN;
      }
    }
    ...
    /* {Summary: Retrieves the unit string for a specific sensor based on its type} */
    String getSensorUnit(int index) {
      ...
      switch (_config.sensors[index].type) {
        ...
        case SENSOR_MODULE_VALUE:
          // Unit is stored in the module's scaleUnit field
          return String(_config.sensors[index].module.scaleUnit);

        case SENSOR_DIGITAL:
          // Digital sensors typically don't have units
          return "";
    ...
    /* {Summary: Finds the array index of a sensor based on its configured name} */
    int getSensorIndexByName(const String& name) const {
      for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
        // Sjekk om slotten er konfigurert og om navnet matcher
        if (_config.sensors[i].type != SENSOR_UNCONFIGURED && String(_config.sensors[i].name) == name) {
          return i;
        }
      }
      return -1; // Return -1 when the sensor does not exist.
    }
    ...
    float retrieveLastValueByName(const String& name) const {
      for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
        ...
        if (String(s.name) == name) {
          return _currentReadings[i];  // standard lookup — returns the current reading
        }

        // Counter pins exposed as virtual signals: "MyCounter.up", "MyCounter.down", "MyCounter.reset"
        if (s.type == SENSOR_COUNTER) {
          if (String(s.name) + ".up"    == name && s.counter.upPin    >= 0)
            return (float)digitalRead(s.counter.upPin);
          if (String(s.name) + ".down"  == name && s.counter.downPin  >= 0)
            return (float)digitalRead(s.counter.downPin);
          if (String(s.name) + ".reset" == name && s.counter.resetPin >= 0)
            return (float)digitalRead(s.counter.resetPin);
        }
      }
      return NAN;
    }
  ...
  private:
    ...
    */
    ...
    float readCounter(int index) {
      SensorConfig& cfg = _config.sensors[index];

      // Read pins
      int upPin    = cfg.counter.upPin;
      int downPin  = cfg.counter.downPin;
      int resetPin = cfg.counter.resetPin;

      // Get acutal count from reserved (RAM-mirrored in lastMeasurment)
      // Lastmeasurment used as live RAM-verdi in runtime
      float count = _currentReadings[index];
      if (isnan(count)) count = loadCountFromSensor(cfg);

      // Stored last edge state in _reserved[5..6]
      bool prevUp   = cfg._reserved[5] & 0x01;
      bool prevDown = cfg._reserved[5] & 0x02;

      bool curUp    = (upPin   >= 0) ? (bool)digitalRead(upPin)   : false;
      bool curDown  = (downPin >= 0) ? (bool)digitalRead(downPin) : false;
      bool curReset = (resetPin >= 0) ? (bool)digitalRead(resetPin) : false;

      // Reset tar prioritet
      if (curReset) {
        count = 0.0f;
        cfg._reserved[5] = 0;
        persistCountIfNeeded(index, count, true);  // tving skriving ved reset
        _currentReadings[index] = count;
        return count;
      }

      // Rising flank up
      if (!prevUp && curUp)     count += 1.0f;
      // Rising flank down
      if (!prevDown && curDown) count -= 1.0f;

      // Save edgestate
      cfg._reserved[5] = ((curUp ? 1 : 0) | (curDown ? 2 : 0));

      _currentReadings[index] = count;
      persistCountIfNeeded(index, count, false);
      return count;
    }

    /* Reads a module sensor using raw Grefur addresses, then applies user scaling. */
    float readModule(int index) {
      SensorConfig& cfg = _config.sensors[index];

#if DEBUG_MODULE_SENSOR
      Serial.println("--- Module Read Start [" + String(cfg.name) + "] ---");
      Serial.printf("Target -> Addr: 0x%02X, Reg: 0x%02X\n", cfg.module.i2cAddress, cfg.module.registerIndex);
#endif

      // 1. Update the library's internal cache from the physical hardware
      if (!ModuleEngine.pollAddress(cfg.module.i2cAddress, cfg.module.registerIndex)) {
        if (cfg.module.errorCount < 255) cfg.module.errorCount++;

#if DEBUG_MODULE_SENSOR
        Serial.printf("I2C Poll Failed! Error Count: %d\n", cfg.module.errorCount);
        Serial.println("--- Module Read End ---");
#endif
        return 0.0f;
      }

      // 2. Clear error count on success
      cfg.module.errorCount = 0;

      // 3. Get the converted float directly from the library
      float rawValue = ModuleEngine.getValueAsFloat(cfg.module.i2cAddress, cfg.module.registerIndex);

      // 4. Calculate final value
      float finalValue = (rawValue * cfg.module.scale) + cfg.module.offset;

#if DEBUG_MODULE_SENSOR
      Serial.printf("Raw Value: %.4f\n", rawValue);
      Serial.printf("Scaling: (%.4f * %.4f) + %.4f\n", rawValue, cfg.module.scale, cfg.module.offset);
      Serial.printf("Final Value: %.4f\n", finalValue);
      Serial.println("--- Module Read End ---");
#endif
    ...
    void persistCountIfNeeded(int index, float count, bool force) {
      SensorConfig& cfg = _config.sensors[index];
      if (!cfg.counter.persistent) return;

      // Dirty-timer ligger i _reserved[6..9] som uint32_t (millisekunder)
      uint32_t& dirtyTime = *(uint32_t*)(&cfg._reserved[6]);
      float lastSaved = loadCountFromSensor(cfg);

      if (count == lastSaved && !force) return;

      uint32_t now = millis();
      if (dirtyTime == 0) dirtyTime = now;

      if (force || (now - dirtyTime >= 5000)) {
        saveCountToSensor(cfg, (int32_t)count);
        dirtyTime = 0;
        _changed = true;   // trigger saveConfig() i neste loop
      }
    }

    void saveCountToSensor(SensorConfig& cfg, int32_t value) {
      cfg._reserved[0] = 0xA5;
      cfg._reserved[1] = (value >> 24) & 0xFF;
      cfg._reserved[2] = (value >> 16) & 0xFF;
      cfg._reserved[3] = (value >>  8) & 0xFF;
      cfg._reserved[4] = (value      ) & 0xFF;
    }

    float loadCountFromSensor(const SensorConfig& cfg) {
      if (cfg._reserved[0] != 0xA5) return 0.0f;
      int32_t val = ((int32_t)cfg._reserved[1] << 24)
                    | ((int32_t)cfg._reserved[2] << 16)
                    | ((int32_t)cfg._reserved[3] <<  8)
                    |  (int32_t)cfg._reserved[4];
      return (float)val;
    }
};
...
// Middleware to take measurements from all configured sensors.
...
class ExpressionManager {
  ...
  public:
    ...
    */
    ...
    void initRetainReadings() {
      for (int i = 0; i < MAX_CALCULATIONS_ALLOWED; i++) {
        if (_configs[i].type == CALC_RETAIN) {
          _runtime[i].lastValue = loadRetainValue(i);
        }
      }
    }
  ...
  private:
    ...
    */
    ...
    void handleRetain(int i) {
      auto& cfg = _configs[i];

      // Fetch value from named calculation source
      float newValue = getCalculationValueByName(String(cfg.retain.sourceTag));

      // Fall back to sensor lookup if not found as a calculation
      if (isnan(newValue))
        newValue = BindingEngine(String(cfg.retain.sourceTag));

      if (isnan(newValue)) return;

      // No change — nothing to persist
      if (newValue == _runtime[i].lastValue) return;

      _runtime[i].lastValue = newValue;
      persistRetainValue(i, newValue);
    }
    ...
    float getValueOrSensor(const String& token) {
      ...
      // Property access via dot notation: "MyDelay.timeLeft", "MyRamp.delta" etc.
      int dotIdx = t.lastIndexOf('.');
      if (dotIdx > 0) {
        String baseName = t.substring(0, dotIdx);
        String prop     = t.substring(dotIdx + 1);
        prop.toUpperCase();

        // Check for sensor properties first
        if (prop == "ERR" || prop == "ERRORCOUNT") {
          int sIdx = sensorManager.getSensorIndexByName(baseName);
          if (sIdx != -1) {
            return (float)config.sensors[sIdx].module.errorCount;
          }
        }

        // Find the named calculation slot
        for (int i = 0; i < _numConfigs; i++) {
          if (_configs[i].type == 0) continue;
          if (String(_configs[i].name) != baseName) continue;

          RuntimeState& rs = _runtime[i];

          // --- DELAY properties ---
          // timeLeft: ms remaining until output changes
          if (prop == "TIMELEFT") {
            if (!rs.delayInitialized || rs.delayStartTime == 0) return 0.0f;
            uint32_t elapsed = millis() - rs.delayStartTime;
            uint32_t window  = 0;
            // Try to extract the window from the expression — read delayTargetValue as proxy
            // delayStartTime is set when timer starts, lastProcessTime holds window in DELAY
            return fmax(0.0f, (float)rs.lastProcessTime - (float)elapsed);
          }

          // active: 1 if the delay timer is currently running
          if (prop == "ACTIVE") {
            return (rs.delayInitialized && rs.delayStartTime > 0) ? 1.0f : 0.0f;
          }

          // --- RAMP properties ---
          // current: current ramp output value
          if (prop == "CURRENT") return rs.lastValue;

          // delta: difference between target and current (how far left to ramp)
          if (prop == "DELTA") return rs.delayTargetValue - rs.lastValue;

          // --- RATE properties ---
          // edgeCount: raw edge count in current window
          if (prop == "EDGECOUNT") return rs.delayTargetValue;

          // windowAge: ms since window started
          if (prop == "WINDOWAGE") return (float)(millis() - rs.delayStartTime);

          // --- INTEGRATE properties ---
          // total: current accumulated value (same as lastValue, but explicit)
          if (prop == "TOTAL") return rs.lastValue;

          // runTime: ms since integrator last reset
          if (prop == "RUNTIME") return (float)(millis() - rs.lastProcessTime);

          // --- PID properties ---
          if (prop == "ERROR")    return rs.lastError;
          if (prop == "INTEGRAL") return rs.integralSum;

          // --- Generic ---
          if (prop == "VALUE")    return rs.lastValue;
        }
      }

      // Standard numeric or sensor lookup
      char* endptr;
    ...
    */
    float evalSimpleExpression(const String& expr, RuntimeState* state) {
      ...
      // SR Latch: SR(setInput, resetInput)
      // Returns Q. Requires state->lastValue to hold Q (NaN = uninitialized).
      if (s.startsWith("SR(") && s.endsWith(")")) {
        ...
        if (comma != -1 && state) {
          float S = evalExpression(inner.substring(0, comma), state);
          float R = evalExpression(inner.substring(comma + 1), state);

          // Initialize to Q=0 on first run
          if (isnan(state->lastValue)) state->lastValue = 0.0f;

          float Q = state->lastValue;

          if (S != 0.0f && R == 0.0f) Q = 1.0f;       // Set
          else if (S == 0.0f && R != 0.0f) Q = 0.0f;  // Reset
          // S=0, R=0 → hold (Q unchanged)
          // S=1, R=1 → invalid, treat as hold (or define behavior as needed)
      ...
      // COUNT(up, down, reset)
      // - Increments by 1 on each rising edge of 'up'
      // - Decrements by 1 on each rising edge of 'down'
      // - Resets to 0 immediately while 'reset' is high
      //
      // State usage:
      //   lastValue        = current count
      //   delayTargetValue = packed previous edge state (up in bit0, down in bit1)
      //   delayInitialized = first-run flag
      ...
      if (s.startsWith("COUNT(") && s.endsWith(")")) {
        ...
        if (state && argCount >= 1) {
          float up    = args[0];
          float down  = (argCount >= 2) ? args[1] : 0.0f;
          float reset = (argCount >= 3) ? args[2] : 0.0f;

          if (!state->delayInitialized) {
            state->delayInitialized  = true;
            state->lastValue         = 0.0f;
            // Pack both previous levels into one float using bit encoding
            // prevUp in bit0, prevDown in bit1  →  stored as 0.0 / 1.0 / 2.0 / 3.0
            uint8_t packed = ((up != 0.0f) ? 1 : 0) | ((down != 0.0f) ? 2 : 0);
            state->delayTargetValue = (float)packed;
            return 0.0f;
          }

          // Decode previous levels
          uint8_t prev     = (uint8_t)state->delayTargetValue;
          bool prevUp      = (prev & 1) != 0;
          bool prevDown    = (prev & 2) != 0;

          // Store new levels
          uint8_t packed = ((up != 0.0f) ? 1 : 0) | ((down != 0.0f) ? 2 : 0);
          state->delayTargetValue = (float)packed;

          // Reset is level-sensitive, takes priority
          if (reset != 0.0f) {
            state->lastValue = 0.0f;
            return 0.0f;
          }
          ...
          // Rising edge on up
          if (!prevUp && up != 0.0f)
            state->lastValue += 1.0f;

          // Rising edge on down
          if (!prevDown && down != 0.0f)
            state->lastValue -= 1.0f;
      ...
      // RATE(input, windowMs) — returns rising edges per second over a sliding window
      // Uses state fields:
      //   lastValue        = current rate (edges/sec)
      //   delayStartTime   = window start timestamp
      //   delayTargetValue = edge count within current window
      //   lastProcessTime  = previous input level (for edge detection)
      //   delayInitialized = first-run flag

      if (s.startsWith("RATE(") && s.endsWith(")")) {
        ...
        if (comma != -1 && state) {
          float input      = evalExpression(inner.substring(0, comma), state);
          uint32_t windowMs = (uint32_t)evalExpression(inner.substring(comma + 1), state);
          uint32_t now     = millis();

          if (!state->delayInitialized) {
            state->delayInitialized  = true;
            state->delayStartTime    = now;
            state->delayTargetValue  = 0.0f;   // edge counter
            state->lastProcessTime   = now;
            state->lastValue         = 0.0f;   // last known rate
            state->lastError         = (input != 0.0f) ? 1.0f : 0.0f;  // prev input level
            return 0.0f;
          }

          // Rising edge detection
          bool prevHigh = (state->lastError > 0.5f);
          bool curHigh  = (input > 0.5f);
          if (!prevHigh && curHigh)
            state->delayTargetValue += 1.0f;
          state->lastError = curHigh ? 1.0f : 0.0f;

          // When window expires, calculate rate and reset
          uint32_t elapsed = now - state->delayStartTime;
          if (elapsed >= windowMs) {
            float seconds    = elapsed / 1000.0f;
            state->lastValue = state->delayTargetValue / seconds;
            state->delayTargetValue = 0.0f;
            state->delayStartTime   = now;
          }
      ...
      // INTEGRATE(input, resetInput) — accumulates input * dt over time, in per-hour units
      // Returns accumulated value (e.g. kWh if input is kW)
      if (s.startsWith("INTEGRATE(") && s.endsWith(")")) {
        String inner = s.substring(10, s.length() - 1);
        int comma = -1, depth = 0;
        for (int i = 0; i < inner.length(); i++) {
          if (inner[i] == '(') depth++;
          else if (inner[i] == ')') depth--;
        ...
        if (state) {
          float input = evalExpression(comma != -1 ? inner.substring(0, comma) : inner, state);
          float reset = (comma != -1) ? evalExpression(inner.substring(comma + 1), state) : 0.0f;

          uint32_t now = millis();

          if (!state->delayInitialized || reset != 0.0f) {
            state->delayInitialized = true;
            state->lastValue        = 0.0f;
            state->lastProcessTime  = now;
            return 0.0f;
          ...
          // dt i timer (input er typisk i kW → gir kWh)
          float dt = (now - state->lastProcessTime) / 3600000.0f;
          state->lastProcessTime = now;
          state->lastValue += input * dt;
      ...
      if (s.startsWith("HYST(") && s.endsWith(")")) {
        ...
        if (argCount == 3 && state) {
          float val = args[0];            // Current value, for example temperature.
      ...
      if (s.startsWith("PID(") && s.endsWith(")")) {
        ...
        for (int i = 0; i < inner.length() && argCount < 7; i++) {
          if (inner[i] == '(') depth++;
          else if (inner[i] == ')') depth--;
          else if (depth == 0 && inner[i] == ',') {
            args[argCount++] = evalExpression(inner.substring(lastPos, i), state);
        ...
        if (argCount < 7) args[argCount++] = evalExpression(inner.substring(lastPos), state);

        // PID(input, setpoint, Kp, Ti, Td [, min, max])
        // Ti = integral time in seconds (0 = disabled)
        // Td = derivative time in seconds (0 = disabled)
        if (state && argCount >= 5) {
          uint32_t now = millis();
          bool firstRun = (state->lastProcessTime == 0);
          float dt = firstRun ? 0.0f : (now - state->lastProcessTime) / 1000.0f;
          state->lastProcessTime = now;

          float error = args[1] - args[0];  // setpoint - input

          // Accumulate integral only after first run (dt is valid)
          if (!firstRun && dt > 0.0f)
            state->integralSum += error * dt;

          // Output clamp limits (optional 6th and 7th arguments)
          float outMin = (argCount >= 7) ? args[5] : -1e6f;
          float outMax = (argCount >= 7) ? args[6] : +1e6f;

          // Anti-windup: clamp integral to output limits to prevent saturation buildup
          state->integralSum = fmax(outMin, fmin(outMax, state->integralSum));

          // I-term: integral sum divided by Ti (disabled if Ti == 0)
          float iTerm = (args[3] > 0.0f) ? (state->integralSum / args[3]) : 0.0f;

          // D-term: Td * derivative — skipped on first run to avoid spike
          float dTerm = (!firstRun && dt > 0.0f)
                        ? (error - state->lastError) / dt
                        : 0.0f;

          state->lastError = error;

          // ISA standard form: output = Kp * (error + iTerm + Td * dTerm)
          float result = args[2] * (error + iTerm + args[4] * dTerm);

          // Clamp final output to [min, max]
          return fmax(outMin, fmin(outMax, result));
        }
        return 0.0f;
      }
    ...
    */
    ...
    void persistRetainValue(int index, float value) {
      auto& cfg = _configs[index];
      uint32_t now = millis();

      // Start debounce timer on first change
      if (_runtime[index].lastProcessTime == 0)
        _runtime[index].lastProcessTime = now;

      uint32_t debounce = cfg.retain.persistDebounceMs > 0 ? cfg.retain.persistDebounceMs : 5000;

      // Only write once debounce period has elapsed
      if (now - _runtime[index].lastProcessTime >= debounce) {
        cfg.retain.lastPersistedValue = value;
        cfg.retain.lastUpdateTs = now;
        _runtime[index].lastProcessTime = 0;  // reset timer

        // Trigger global EEPROM save
        saveConfig();

#if DEBUG_EXPRESSIONS
        Serial.printf("[Retain] Persisted '%s' = %.2f\n", cfg.name, value);
#endif
      }
    }

    float loadRetainValue(int index) {
      auto& cfg = _configs[index];

      // Return stored value if valid
      if (!isnan(cfg.retain.lastPersistedValue)) {
#if DEBUG_EXPRESSIONS
        Serial.printf("[Retain] Loaded '%s' = %.2f\n", cfg.name, cfg.retain.lastPersistedValue);
#endif
        return cfg.retain.lastPersistedValue;
      }

      // No stored value — start from zero
      return 0.0f;
    }
};
...
class ActuatorManager {
  private:
    ...
    /* Writes a value to a module output by un-scaling and sending via I2C */
    bool writeModule(int index, float value) {
      ActuatorConfig& cfg = _config.actuators[index];

      // 1. Reverse scaling: raw = (value - offset) / scale
      float scaleVal = (cfg.module.scale != 0.0f) ? cfg.module.scale : 1.0f;
      float rawValue = (value - cfg.module.offset) / scaleVal;

      // 2. Pack into GrefurValue union based on register data type
      GrefurValue val;
      uint8_t type = cfg.module.dataType;

      if      (type == GREFUR_T_F32) val.asFloat = rawValue;
      else if (type == GREFUR_T_I32) val.asI32   = (int32_t)roundf(rawValue);
      else if (type == GREFUR_T_U16) val.asU16   = (uint16_t)constrain(rawValue, 0, 65535);
      else if (type == GREFUR_T_I16) val.asI16   = (int16_t)constrain(rawValue, -32768, 32767);
      else                           val.asU8    = (uint8_t)constrain(rawValue, 0, 255);

#if DEBUG_MODULE_OUTPUT
      Serial.printf("--- Module Write [%s] ---\n", cfg.name);
      Serial.printf("  Target: Addr=0x%02X Reg=0x%02X Type=%d\n",
                    cfg.module.i2cAddress, cfg.module.registerAddress, type);
      Serial.printf("  Value: %.4f -> (%.4f - %.4f) / %.4f = Raw: %.4f\n",
                    value, value, cfg.module.offset, scaleVal, rawValue);
#endif

      // 3. Write to hardware
      if (!ModuleEngine.writeRaw(cfg.module.i2cAddress, cfg.module.registerAddress, type, val)) {
        if (cfg.module.errorCount < 255) cfg.module.errorCount++;
#if DEBUG_MODULE_OUTPUT
        Serial.printf("  I2C Write FAILED — Error count: %d\n", cfg.module.errorCount);
#endif
        return false;
      }
  ...
  public:
    ...
    int addActuator(const ActuatorConfig& newActuator) {
      for (int i = 0; i < MAX_ACTUATORS_ALLOWED; i++) {
        if (_config.actuators[i].type == ACTUATOR_UNCONFIGURED) {
          ...
          _config.actuators[i].runtimeValue = NAN;
          _config.actuators[i].lastUpdateTs = 0;
          _config.actuators[i].lastPublishedValue = NAN;
    ...
    /* {Summary: Drives pins and I2C modules according to publishPolicy, ensuring hardware is only updated when thresholds are met} */
    void updateActuators(ExpressionManager& expManager) {
      ...
      for (int i = 0; i < MAX_ACTUATORS_ALLOWED; i++) {
        ...
        // Check for change from alst runtime value
        if (!isnan(act.runtimeValue) && fabsf(targetValue - act.runtimeValue) < 1e-4f) continue;


        // 1. Evaluate publish policy to avoid spamming hardware/I2C
        // NAN runtimeValue (e.g. after save) forces an immediate update
        /*float delta = isnan(act.runtimeValue) ? 1e6f : abs(targetValue - act.runtimeValue);
          uint32_t elapsed = currentTime - act.lastUpdateTs;
          bool shouldUpdate = false;

          switch (act.publishPolicy.trigger) {
          case PUBLISH_ON_THRESHOLD_OR_INTERVAL:
            shouldUpdate = (delta >= act.publishPolicy.changeThreshold) || (elapsed >= act.publishPolicy.intervalMs);
            break;
        ...
        // 2. Drive hardware based on type
        if (act.type == ACTUATOR_DIGITAL) {
          bool pinState = (targetValue > 0.5f);
          if (act.digital.inverted) pinState = !pinState;

          if (!_initializedPins[i]) {
            pinMode(act.digital.digitalPin, OUTPUT);
            _initializedPins[i] = true;
          }
          analogWrite(act.digital.digitalPin, pinState ? 255 : 0);
        }
        else if (act.type == ACTUATOR_ANALOG) {
          float clampedValue = constrain(targetValue, act.analog.minValue, act.analog.maxValue);

          // Map value range to voltage range, then voltage to PWM (3.3V logic)
          float targetVoltage = act.analog.voltageMin + (clampedValue / 100.0f)
                                * (act.analog.voltageMax - act.analog.voltageMin);
          int pwmValue = (int)((targetVoltage / 3.3f) * 255.0f);
          pwmValue = constrain(pwmValue, 0, 255);

          //if (act.analog.inverted) pwmValue = 255 - pwmValue; // Fixed: was act.digital.inverted
        ...
        else if (act.type == ACTUATOR_MODULE_VALUE) {
          writeModule(i, targetValue);
        }
...
/* {Summary: Updated occupancy check to strictly separate sensor and actuator contexts while preventing false positives on pin -1} */
bool isResourceOccupied(int pin, uint8_t addr = 0, uint16_t reg = 0, int currentId = -1, bool isActuator = false) {
  // --- Check all configured sensors ---
  for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
    auto& s = config.sensors[i];
    if (s.type == SENSOR_UNCONFIGURED) continue;

    // Skip if we are currently editing this specific sensor
    if (!isActuator && i == currentId) continue;

    // Only check GPIO pins if a valid pin number (>= 0) is provided
    if (pin >= 0) {
      if (s.type == SENSOR_NTC && s.ntc.analogPin == pin) return true;
      if (s.type == SENSOR_ANALOG && s.analog.analogPin == pin) return true;
      if (s.type == SENSOR_DIGITAL && s.digital.digitalPin == pin) return true;
      if (s.type == SENSOR_COUNTER && (s.counter.upPin == pin || s.counter.downPin == pin || s.counter.resetPin == pin)) return true;
    }

    // Check I2C Module occupancy for sensors
    if (addr > 0 && s.type == SENSOR_MODULE_VALUE && s.module.i2cAddress == addr && s.module.registerIndex == reg) return true;
  }

  // --- Check all configured actuators ---
  for (int i = 0; i < MAX_ACTUATORS_ALLOWED; i++) {
    auto& a = config.actuators[i];
    if (a.type == ACTUATOR_UNCONFIGURED) continue;

    // Skip if we are currently editing this specific actuator
    if (isActuator && i == currentId) continue;

    // Only check GPIO pins if a valid pin number (>= 0) is provided
    if (pin >= 0) {
      if (a.type == ACTUATOR_DIGITAL && a.digital.digitalPin == pin) return true;
      if (a.type == ACTUATOR_ANALOG && a.analog.analogPin == pin) return true;
    }

    // Check I2C Module occupancy for actuators
    if (addr > 0 && a.type == ACTUATOR_MODULE_VALUE && a.module.i2cAddress == addr && a.module.registerAddress == reg) return true;
  }

  return false;
}

String getDiscoveredModulesJson(int currentId = -1, bool isActuator = false, bool writableOnly = false) {
  String json = "[";

  for (uint8_t d = 0; d < ModuleEngine.getDeviceCount(); d++) {
    uint8_t addr = ModuleEngine.getDeviceAddress(d);

    for (uint8_t r = 0; r < ModuleEngine.getRegisterCount(d); r++) {
      if (writableOnly && !ModuleEngine.getDevice(d)->registers[r].writable) continue;

      if (json.length() > 1) json += ",";

      uint16_t reg = ModuleEngine.getRegisterAddress(d, r);
      bool occ = isResourceOccupied(-1, addr, reg, currentId, isActuator);

      json += "{";
      json += "\"name\":\"" + String(ModuleEngine.getRegisterName(d, r)) + "\",";
      json += "\"addr\":" + String(addr) + ",";
      json += "\"reg\":" + String(reg) + ",";
      json += "\"type\":" + String(ModuleEngine.getRegisterDataType(d, r)) + ",";
      json += "\"occ\":" + String(occ ? "true" : "false");
      json += "}";
    }
  }
...
class BackupManager {
  ...
  public:
    ...
    static String generateBackupJsonBase64(const EEPROMLayout& sourceConfig) {
      const uint8_t* ptr = reinterpret_cast<const uint8_t*>(&sourceConfig);
      size_t len = sizeof(EEPROMLayout);

      String b64 = base64::encode(ptr, len);
      String output;
      output.reserve(b64.length() + 768);
      output += "{\n";
      output += "  \"format\": \"grefur-eeprom-backup\",\n";
      output += "  \"encoding\": \"base64-eeprom-layout\",\n";
      output += "  \"firmwareVersion\": \"" + String(FIRMWARE_VERSION) + "\",\n";
      output += "  \"layoutSize\": " + String(len) + ",\n";
      output += "  \"device\": {\n";
      output += "    \"name\": \"" + jsonEscape(sourceConfig.device.deviceName) + "\",\n";
      output += "    \"location\": \"" + jsonEscape(sourceConfig.device.location) + "\",\n";
      output += "    \"accessPoint\": \"" + jsonEscape(sourceConfig.device.apName) + "\",\n";
      output += "    \"productionToken\": \"" + jsonEscape(sourceConfig.device.productionToken) + "\"\n";
      output += "  },\n";
      output += "  \"network\": {\n";
      output += "    \"wifiSSID\": \"" + jsonEscape(sourceConfig.network.wifiSSID) + "\",\n";
      output += "    \"useDHCP\": " + String(sourceConfig.network.useDHCP ? "true" : "false") + "\n";
      output += "  },\n";
      output += "  \"mqtt\": {\n";
      output += "    \"brokerURL\": \"" + jsonEscape(sourceConfig.mqtt.brokerURL) + "\",\n";
      output += "    \"port\": " + String(sourceConfig.mqtt.port) + ",\n";
      output += "    \"baseTopic\": \"" + jsonEscape(sourceConfig.mqtt.baseTopic) + "\",\n";
      output += "    \"useGrefurBackend\": " + String(sourceConfig.mqtt.useGrefurBackend ? "true" : "false") + "\n";
      output += "  },\n";
      output += "  \"counts\": {\n";
      output += "    \"sensors\": " + String(sensorManager.getConfiguredSensorCount()) + ",\n";
      output += "    \"actuators\": " + String(actuatorManager.getConfiguredActuatorCount()) + ",\n";
      output += "    \"calculations\": " + String(expressionManager.getConfiguredCalculationCount()) + "\n";
      output += "  },\n";
      output += "  \"data\": \"" + b64 + "\"\n";
      output += "}\n";
      return output;
    ...
    static bool restoreFromBackupJsonBase64(const String& jsonString) {
      String b64String = extractJsonStringField(jsonString, "data");
      if (b64String.length() == 0) {
        Serial.println("Missing Base64 data");
        return false;
      }

      const char* b64 = b64String.c_str();
...
class CertificateManager {
  private:
    ...
    bool hasPemBlock(const String& value, const char* blockName) const {
      String beginMarker = "-----BEGIN " + String(blockName) + "-----";
      String endMarker = "-----END " + String(blockName) + "-----";
      return value.indexOf(beginMarker) >= 0 && value.indexOf(endMarker) >= 0;
    }

    bool hasPrivateKeyBlock(const String& value) const {
      return value.indexOf("-----BEGIN ") >= 0 &&
             value.indexOf("PRIVATE KEY-----") >= 0 &&
             value.indexOf("-----END ") >= 0;
    }
  ...
  public:
    bool begin() {
#if defined(ESP32)
      _ready = SPIFFS.begin(true);
#else
      _ready = SPIFFS.begin();
#endif

      if (!_ready) {
        Serial.println("[Certificate] Failed to mount SPIFFS");
      }
      return _ready;
    }
    ...
    String saveCertificate(const String& cert, const String& key, const String& rootCA = "") {
      if (!_ready) return "Certificate storage is not ready.";
      if (!hasPemBlock(cert, "CERTIFICATE")) return "Certificate PEM is missing a CERTIFICATE block.";
      if (!hasPrivateKeyBlock(key)) return "Private key PEM is missing a PRIVATE KEY block.";

      if (!saveFile(certPath, cert)) return "Failed to save certificate.";
      if (!saveFile(keyPath, key)) return "Failed to save private key.";

      if (rootCA.length() > 0) {
        if (!hasPemBlock(rootCA, "CERTIFICATE")) return "Root CA PEM is missing a CERTIFICATE block.";
        if (!saveFile(caPath, rootCA)) return "Failed to save root CA.";
      } else if (SPIFFS.exists(caPath)) {
        SPIFFS.remove(caPath);
      }

      return "Certificate and private key saved.";
    }
    ...
    String removeCertificate() {
      ...
      if (SPIFFS.exists(keyPath)) SPIFFS.remove(keyPath);
      if (SPIFFS.exists(caPath)) SPIFFS.remove(caPath);
      return "Certificate files removed.";
    }
    ...
    bool validateCertificate() {
      ...
      if (!certFile || !keyFile) return false;
      ...
      return hasPemBlock(cert, "CERTIFICATE") && hasPrivateKeyBlock(key);
    }

    String getCertificateSummary() {
      String summary;
      summary += "Storage ready: " + String(_ready ? "Yes" : "No") + "\n";
      summary += "Certificate present: " + String(_ready && SPIFFS.exists(certPath) ? "Yes" : "No") + "\n";
      summary += "Private key present: " + String(_ready && SPIFFS.exists(keyPath) ? "Yes" : "No") + "\n";
      summary += "Root CA present: " + String(_ready && SPIFFS.exists(caPath) ? "Yes" : "No") + "\n";
      summary += "PEM format valid: " + String(validateCertificate() ? "Yes" : "No") + "\n";
      return summary;
    }

    String loadCertificate() {
      if (!_ready || !SPIFFS.exists(certPath)) return "";
      File f = SPIFFS.open(certPath, "r");
      if (!f) return "";
      String content = f.readString();
      f.close();
    ...
    String loadKey() {
      if (!_ready || !SPIFFS.exists(keyPath)) return "";
      File f = SPIFFS.open(keyPath, "r");
      if (!f) return "";
      String content = f.readString();
      f.close();
      return content;
    ...
    String loadRootCA() {
      if (!_ready || !SPIFFS.exists(caPath)) return "";
      File f = SPIFFS.open(caPath, "r");
      if (!f) return "";
      String content = f.readString();
      f.close();
      return content;
...
class CertificateManager {
  public:
    ...
    String saveCertificate(const String&, const String&, const String& = "") {
      return "Certificate storage is not supported on this board.";
    }
    String removeCertificate() {
      return "Certificate storage is not supported on this board.";
    }
...
class DisplayManager {
  private:
    ...
    // Bind-engine: callback-type for å hente verdi fra RAM
    // Settes av eier (f.eks. main.ino) via setBindEngine()
    float (*_bindEngine)(const String& sensorName) = nullptr;

  public:

    DisplayManager(EEPROMLayout& config)
      : _config(config),
    ...
    bool initScreen() {
      if (!_display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDR)) {
        Serial.println("[Display] OLED not found at 0x3C!");
        return false;
      }
      ...
      // Vis splash mens resten av systemet starter
      _display.setTextSize(1);
      _display.setCursor(28, 20);
      _display.print("Starter opp...");
      _display.display();

      // Validate the stored design.
      if (_config.screen.magic != 0xD1) {
        Serial.println("[Display] No valid design in EEPROM. Loading defaults.");
        _loadDefaultDesign();
      }

      Serial.printf("[Display] Init OK. %d widgets lastet.\n",
                    _config.screen.widgetCount);
      return true;
    }

    // Connect the binding engine after display initialization.
    //    Eksempel: display.setBindEngine([](const String& name) {
    //                return sensorManager.retrieveLastValueByName(name);
    //              });
    void setBindEngine(float (*fn)(const String&)) {
      _bindEngine = fn;
    }

    // Update loop. Called from loop().
    void updateScreen() {
      uint16_t interval = (_config.screen.refreshMs > 0)
                          ? _config.screen.refreshMs : 1000;

      if (millis() - _lastRefresh < interval) return;
      _lastRefresh = millis();

      _display.clearDisplay();
      _renderDesign(_config.screen);
      _display.display();
    }

    // ── REST API: hva slags widgets støttes ─────────────────────
    String getScreenFeatures() {
      DynamicJsonDocument doc(512);
      JsonArray features = doc.createNestedArray("widgets");

      auto addFeature = [&](const char* id, const char* label,
      const char* description, JsonArray & fields_arr) {
        JsonObject w = features.createNestedObject();
        w["id"]          = id;
        w["label"]       = label;
...

      {
        JsonObject w = features.createNestedObject();
        w["id"]          = "label";
        w["label"]       = "Statisk tekst";
        w["description"] = "Vis fast tekst på skjermen";
        JsonArray f = w.createNestedArray("fields");
        f.add("x"); f.add("y"); f.add("textSize"); f.add("staticText");
      }
      {
        JsonObject w = features.createNestedObject();
        w["id"]          = "value";
        w["label"]       = "Sensorverdi";
        w["description"] = "Live verdi bundet til et sensornavn";
        JsonArray f = w.createNestedArray("fields");
        f.add("x"); f.add("y"); f.add("textSize");
        f.add("sensorName"); f.add("decimals");
      }
      {
        JsonObject w = features.createNestedObject();
        w["id"]          = "unit";
        w["label"]       = "Unit";
        w["description"] = "Sensor unit, for example C";
        JsonArray f = w.createNestedArray("fields");
        f.add("x"); f.add("y"); f.add("textSize"); f.add("sensorName");
      }
      {
        JsonObject w = features.createNestedObject();
        w["id"]          = "bar";
        w["label"]       = "Progressbar";
        w["description"] = "Horisontal bar som skalerer mellom min og maks";
        JsonArray f = w.createNestedArray("fields");
        f.add("x"); f.add("y"); f.add("sensorName");
        f.add("barMin"); f.add("barMax"); f.add("barWidth");
      }
      {
        JsonObject w = features.createNestedObject();
        w["id"]          = "divider";
        w["label"]       = "Skillelinje";
        w["description"] = "Horisontal linje på gitt y-posisjon";
        JsonArray f = w.createNestedArray("fields");
        f.add("y");
      }
      {
        ...
        w["id"]          = "clock";
        w["label"]       = "Klokke";
        w["description"] = "Viser oppetid (eller RTC hvis tilgjengelig)";
        JsonArray f = w.createNestedArray("fields");
        f.add("x"); f.add("y"); f.add("textSize");
      }
      ...
      screen["width"]  = OLED_WIDTH;
      screen["height"] = OLED_HEIGHT;
      screen["maxWidgets"] = MAX_SCREEN_WIDGETS;

      // Tekstst rrelser
      JsonArray sizes = doc.createNestedArray("textSizes");
      sizes.add(1); sizes.add(2); sizes.add(3);
    ...
    // ── REST API: hent gjeldende design ─────────────────────────
    String getScreenDesign() {
      DynamicJsonDocument doc(1024);

      doc["magic"]     = _config.screen.magic;
      doc["title"]     = _config.screen.title;
      doc["refreshMs"] = _config.screen.refreshMs;

      JsonArray widgets = doc.createNestedArray("widgets");
      for (int i = 0; i < _config.screen.widgetCount; i++) {
        const ScreenWidget& w = _config.screen.widgets[i];
        JsonObject obj = widgets.createNestedObject();
        obj["type"]       = w.type;
        obj["x"]          = w.x;
        obj["y"]          = w.y;
        obj["textSize"]   = w.textSize;
        obj["sensorName"] = w.sensorName;
        obj["staticText"] = w.staticText;
        obj["barMin"]     = w.barMin;
        obj["barMax"]     = w.barMax;
        obj["barWidth"]   = w.barWidth;
        obj["decimals"]   = w.decimals;
      }
    ...
    // ── REST API: lagre nytt eller oppdatert design ──────────────
    bool saveScreenDesign(const String& json) {
      DynamicJsonDocument doc(1024);
      ...
      ScreenDesign newDesign = {};
      newDesign.magic      = 0xD1;
      newDesign.refreshMs  = doc["refreshMs"] | 1000;

      const char* title = doc["title"] | "";
      strncpy(newDesign.title, title, sizeof(newDesign.title) - 1);

      JsonArray widgets = doc["widgets"];
      newDesign.widgetCount = 0;

      for (JsonObject obj : widgets) {
        if (newDesign.widgetCount >= MAX_SCREEN_WIDGETS) break;

        ScreenWidget& w = newDesign.widgets[newDesign.widgetCount];
        w.type     = obj["type"]     | (int8_t)WIDGET_NONE;
        w.x        = obj["x"]        | 0;
        w.y        = obj["y"]        | 0;
        w.textSize = obj["textSize"] | 1;
        w.barMin   = obj["barMin"]   | 0.0f;
        w.barMax   = obj["barMax"]   | 100.0f;
        w.barWidth = obj["barWidth"] | 80;
        w.decimals = obj["decimals"] | 1;

        const char* sn = obj["sensorName"] | "";
        strncpy(w.sensorName, sn, sizeof(w.sensorName) - 1);

        const char* st = obj["staticText"] | "";
        strncpy(w.staticText, st, sizeof(w.staticText) - 1);

        newDesign.widgetCount++;
      }

      memcpy(&_config.screen, &newDesign, sizeof(ScreenDesign));
      _changed = true;

      Serial.printf("[Display] New design saved: %d widgets\n",
                    _config.screen.widgetCount);
      return true;
    }
    ...
    bool saveChanges() {
      ...
      _changed = false;
      if (saveConfig()) {
        Serial.println("[Display] Design saved to EEPROM.");
        return true;
      }
      _changed = true;
      return false;
  ...
  private:
    ...
    // ── Render alle widgets fra et design ───────────────────────
    void _renderDesign(const ScreenDesign& design) {
      for (int i = 0; i < design.widgetCount; i++) {
        _renderWidget(design.widgets[i]);
      }
    }

    void _renderWidget(const ScreenWidget& w) {
      _display.setTextSize(w.textSize);

      switch (w.type) {

        case WIDGET_LABEL:
          _display.setCursor(w.x, w.y);
          _display.print(w.staticText);
          break;

        case WIDGET_VALUE: {
            float val = _resolveValue(w.sensorName);
            _display.setCursor(w.x, w.y);
            if (!isnan(val)) {
              _display.print(val, w.decimals);
            } else {
              _display.print("--");
            }
            break;
        ...
        case WIDGET_UNIT:
          // Units are resolved through the binding convention: "name.unit".
          // The owner can also register a unit callback.
          // Use sensorName as fallback unit text when no binding is configured.
          _display.setCursor(w.x, w.y);
          _display.print(w.staticText[0] ? w.staticText : w.sensorName);
          break;
        ...
        case WIDGET_BAR: {
            ...
            if (!isnan(val)) {
              float pct = constrain(
                            (val - w.barMin) / (w.barMax - w.barMin), 0.0f, 1.0f
                          );
              uint8_t filled = (uint8_t)(pct * w.barWidth);
              _display.drawRect(w.x, w.y, w.barWidth, 8, SSD1306_WHITE);
              if (filled > 0) {
                _display.fillRect(w.x, w.y, filled, 8, SSD1306_WHITE);
              }
            }
            break;
          }
        ...
        case WIDGET_DIVIDER:
          _display.drawFastHLine(0, w.y, OLED_WIDTH, SSD1306_WHITE);
          break;
    ...
    // ── Bind-engine: hent live verdi fra RAM ────────────────────
    float _resolveValue(const char* sensorName) {
      if (_bindEngine == nullptr || sensorName[0] == '\0') return NAN;
      return _bindEngine(String(sensorName));
    }

    // ── Standarddesign hvis EEPROM er tom ───────────────────────
    void _loadDefaultDesign() {
      ScreenDesign d = {};
      d.magic      = 0xD1;
      d.refreshMs  = 1000;
      strncpy(d.title, "Standard", sizeof(d.title) - 1);
      d.widgetCount = 3;

      // Widget 0: tittel
      d.widgets[0].type     = WIDGET_LABEL;
      d.widgets[0].x        = 0;
      d.widgets[0].y        = 0;
      d.widgets[0].textSize = TEXT_SMALL;
      strncpy(d.widgets[0].staticText, "Temperatur", 10);

      // Widget 1: skillelinje
      d.widgets[1].type = WIDGET_DIVIDER;
      d.widgets[1].y    = 12;

      // Widget 2: sensorverdi (bundet til første sensor)
      d.widgets[2].type       = WIDGET_VALUE;
      d.widgets[2].x          = 10;
      d.widgets[2].y          = 24;
      d.widgets[2].textSize   = TEXT_LARGE;
      d.widgets[2].decimals   = 1;
      strncpy(d.widgets[2].sensorName, "sensor0", 7);

      memcpy(&_config.screen, &d, sizeof(ScreenDesign));
    }
};

// Global instans (lik SensorManager-mønsteret)
...
/* {Summary: Generates a professional upgrade notice using existing system styles for a seamless brand experience} */
String DisplaySellUpgrades(String resourceName) {
  String html;

  // Create a lowercase version safely for the body text
  String resourceLower = resourceName;
  resourceLower.toLowerCase();

  html += "<div class='upgrade-notice' style='margin: 20px 0; padding: 25px; border: 1px solid #eee; border: 2px solid var(--primary); background: #fafafa; text-align: center;'>";

  // Header
  html += "  <h3 style='margin: 0 0 10px 0; color: var(--dark-text); font-weight: 600;'>";
  html += "    Maximum " + resourceName + " Capacity Reached";
  html += "  </h3>";

  // Body text
  html += "  <p style='margin: 0 auto 20px auto; max-width: 500px; line-height: 1.6; font-size: 16px; color: #555;'>";
  html += "    Your current hardware configuration has reached its limit for " + resourceLower + "s. ";
  html += "    To expand your system with more capabilities, please reach out to our team for an upgrade.";
  html += "  </p>";

  // Action Buttons using existing CSS classes
  html += "  <div style='display: flex; gap: 12px; justify-content: center; flex-wrap: wrap;'>";

  // Main Action: Contact Admin (using your primary button style)
  html += "    <a href='mailto:admin@grefur.no?subject=Upgrade%20Request%20-%20" + resourceName + "' ";
  html += "       class='inheritSubmitStyle' style='text-decoration: none; display: inline-block; min-width: 200px;'>";
  html += "       Contact System Admin";
  html += "    </a>";

  // Secondary Action: Solutions (using your primary link style)
  html += "    <a href='https://grefur.no/solutions' target='_blank' ";
  html += "       style='padding: 10px 20px; color: var(--primary); text-decoration: none; font-weight: 600; min-width: 150px;'>";
  html += "       Explore Solutions &rarr;";
  html += "    </a>";
...
/* {Summary: Generates a minimalist resource slot tracker and the global JS engine.
   Integrates DisplaySellUpgrades dynamically when capacity is reached.} */
String getGrefurResourceManager(String prefix, int current, int max, String label) {
  String html = "";

  // --- 1. HTML COMPONENT ---
  html += "<div class='resource-manager' id='" + prefix + "ResContainer' style='margin-bottom:20px; padding: 12px; border-bottom: 2px solid rgba(0,0,0,0.05);'>";
  html += "  <div style='display:flex; justify-content:space-between; align-items:center;'>";
  //html += "    <span style='font-weight:700; color:var(--text-primary); text-transform: uppercase; letter-spacing: 0.5px; font-size:0.85em;'>" + label + "</span>";
  html += "    <div style='background:rgba(0,0,0,0.05); padding: 4px 10px; border-radius: 4px;'>";
  html += "      <span id='" + prefix + "SlotsAvail' style='font-weight:800; color:var(--primary);'>" + String(max - current) + "</span>";
  html += "      <span style='font-size:0.8em; opacity:0.6; margin: 0 2px;'>AVAILABLE OF</span>";
  html += "      <span id='" + prefix + "SlotsMax' style='font-weight:600;'>" + String(max) + "</span>";
  html += "    </div>";
  html += "  </div>";

  /* Slot warning area - initially populated if current >= max */
  html += "  <div id='" + prefix + "SlotWarning' style='margin-top:8px;'>";
  if (current >= max) {
    html += DisplaySellUpgrades(label);
  }
  html += "  </div>";
  html += "</div>";
  ...
  // --- 2. JAVASCRIPT ENGINE ---
  html += "<script>";
  html += "if (typeof syncGrefurResource !== 'function') {";
  html += "  function syncGrefurResource(prefix, currentCount, label) {";
  html += "    const availEl = document.getElementById(prefix + 'SlotsAvail');";
  html += "    const maxEl = document.getElementById(prefix + 'SlotsMax');";
  html += "    const warnEl = document.getElementById(prefix + 'SlotWarning');";
  html += "    const targetForm = document.getElementById(prefix + 'Form') || document.getElementById(prefix + 'form');";

  html += "    if (!availEl || !maxEl) return false;";

  html += "    const max = parseInt(maxEl.innerText);";
  html += "    const avail = Math.max(0, max - currentCount);";
  html += "    availEl.innerText = avail;";

  /* Template for the upgrade UI box */
  html += "    const upgradeHtml = `" + DisplaySellUpgrades("${label}") + "`;";

  html += "    if (avail <= 0) {";
  html += "      if (warnEl) warnEl.innerHTML = upgradeHtml.replace('${label}', label);";
  html += "      if (targetForm) targetForm.style.display = 'none';";
  html += "    } else {";
  html += "      if (warnEl) warnEl.innerHTML = '';";
  html += "      if (targetForm) targetForm.style.display = 'flex';";
  html += "    }";
  html += "    return avail > 0;";
  html += "  }";
  html += "}";
...
String GrefurServicesScript() {
  String html;

  html += "<script>";
  html += "document.querySelector('input[name=useGrefur]').addEventListener('change', function() {";
  html += "  const locked = this.checked;";
  html += "  const fields = ['mqttServer','mqttPort','mqttClientId','mqttUser','mqttPassword','mqttQos','mqttRetain','useNestedJson'];";
  html += "  fields.forEach(id => {";
  html += "    const el = document.getElementById(id);";
  html += "    if (!el) return;";
  html += "    if (locked) {";
  html += "      el.setAttribute('disabled', true);";
  html += "      el.setAttribute('readonly', true);";
  html += "      el.style.backgroundColor = '#f0f0f0';";
  html += "      el.style.color = '#666';";
  html += "    } else {";
  html += "      el.removeAttribute('disabled');";
  html += "      el.removeAttribute('readonly');";
  html += "      el.style.backgroundColor = '';";
  html += "      el.style.color = '';";
  html += "    }";
  html += "  });";
  html += "});";
...
/* ============================================================
   getSensorFormFields()
   Drop-in replacement for the old SensorTypeFieldsScript().
   Follows the same pattern as getActuatorFormFields():
     - One central JS function (toggleSensorFields)
     - All type-specific HTML built inline as a string `h`
     - container.innerHTML = h at the end
     - Supports optional SensorConfig* for pre-populating edit pages
   ============================================================ */
String getSensorFormFields(const SensorConfig* sen = nullptr) {

  /* ── Identify current sensor index (prevents self-blocking on edit) ── */
  int currentSensorId = -1;
  if (sen != nullptr) {
    for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
      if (sen == &config.sensors[i]) {
        currentSensorId = i;
        break;
      }
    }
  ...
  /* ── Create default config if called from Add page ── */
  SensorConfig d;
  if (!sen) {
    d.type = SENSOR_NTC;
    memset(d.name, 0, sizeof(d.name));
    d.ntc.analogPin      = 0;
    d.ntc.seriesResistor = 10000;
    d.ntc.nominalResistance = 10000;
    d.ntc.nominalTemp    = 25;
    d.ntc.bCoefficient   = 3435;
    strncpy(d.ntc.unit, "°C", sizeof(d.ntc.unit));
    d.analog.analogPin   = 0;
    d.analog.inputMin    = 0;
    d.analog.inputMax    = 4095;
    d.analog.scaleMin    = 0;
    d.analog.scaleMax    = 100;
    memset(d.analog.scaleUnit, 0, sizeof(d.analog.scaleUnit));
    d.digital.digitalPin = 0;
    d.digital.inverted   = false;
    d.counter.upPin      = 0;
    d.counter.downPin    = -1;
    d.counter.resetPin   = -1;
    d.counter.persistent = false;
    d.module.i2cAddress  = 0;
    d.module.registerIndex = 0;
    d.module.dataType    = 0;
    d.module.scale       = 1.0f;
    memset(d.module.scaleUnit, 0, sizeof(d.module.scaleUnit));
    d.publishPolicy.trigger         = PUBLISH_ON_INTERVAL;
    d.publishPolicy.changeThreshold = 0.5f;
    d.publishPolicy.intervalMs      = 60000;
    sen = &d;
  }

  String html = "";

  /* ── Sensor Type dropdown (shown on Add page; hidden on Edit page) ── */
  if (currentSensorId == -1) {
    html += "<div><label>Sensor Type</label>";
    html += "<select id='sensorType' name='type' onchange='toggleSensorFields()' required>";
    html += "<option value=''>-- Choose a type --</option>";

    String typesJson = sensorManager.retrieveSensorTypes();
    DynamicJsonDocument jsonDoc(512);
    DeserializationError error = deserializeJson(jsonDoc, typesJson);
    if (!error && jsonDoc.containsKey("types")) {
      for (JsonVariant typeVar : jsonDoc["types"].as<JsonArray>()) {
        String typeLabel = typeVar.as<String>();
        String typeValue = typeLabel;
        typeValue.toLowerCase();
        html += "<option value='" + typeValue + "'>" + typeLabel + "</option>";
      }
    } else {
      html += "<option value='ntc'>NTC</option>";
      html += "<option value='analog'>Analog</option>";
      html += "<option value='digital'>Digital</option>";
      html += "<option value='counter'>Counter</option>";
      html += "<option value='grefur module'>Grefur Module</option>";
    }
    html += "</select></div>";

    /* Sensor name (only on Add page; Edit page passes it as hidden) */
    html += "<div><label>Sensor Name</label>";
    html += "<input type='text' name='name' placeholder='e.g. Living Room Temp' required></div>";
  }

  /* ── Dynamic fields container ── */
  html += "<div id='dynamicSensorFields' style='display:none'></div>";

  /* ── Build pin JSON (occupancy-aware, same logic as actuator) ── */
  html += "<script>";

  html += "var sensorPins = [";
  for (int i = 0; i < 9; i++) {
    bool occ = isResourceOccupied(validGpioPins[i], 0, 0, currentSensorId, false);
    bool isCurrent = false;
    if (sen->type == SENSOR_NTC     && validGpioPins[i] == sen->ntc.analogPin)     isCurrent = true;
    if (sen->type == SENSOR_ANALOG  && validGpioPins[i] == sen->analog.analogPin)  isCurrent = true;
    if (sen->type == SENSOR_DIGITAL && validGpioPins[i] == sen->digital.digitalPin) isCurrent = true;
    if (sen->type == SENSOR_COUNTER && validGpioPins[i] == sen->counter.upPin)     isCurrent = true;
    if (i > 0) html += ",";
    html += "{p:" + String(validGpioPins[i]) + ",occ:" + (occ ? "true" : "false") + ",cur:" + (isCurrent ? "true" : "false") + "}";
  }
  html += "];";

  /* Helper: build <option> list for a pin select */
  html += "var getPinOpts=(cur)=>sensorPins.map(p=>{"
          "  const dis=(p.occ&&p.p!=cur)?'disabled':'';"
          "  const lbl=(p.occ&&p.p!=cur)?`GPIO ${p.p} (In Use)`:`GPIO ${p.p}`;"
          "  const sel=(p.p==cur)?'selected':'';"
          "  return `<option value='${p.p}' ${sel} ${dis}>${lbl}</option>`;"
          "}).join('');";

  /* ── Module discovery JSON ── */
  html += "var sensorModules = " + getDiscoveredModulesJson(currentSensorId, false, false) + ";";

  /* ── Saved values baked in from C++ for pre-population on Edit ── */
  String savedType;
  if      (sen->type == SENSOR_NTC)          savedType = "ntc";
  else if (sen->type == SENSOR_ANALOG)        savedType = "analog";
  else if (sen->type == SENSOR_DIGITAL)       savedType = "digital";
  else if (sen->type == SENSOR_COUNTER)       savedType = "counter";
  else if (sen->type == SENSOR_MODULE_VALUE)  savedType = "module";
  else                                        savedType = "";

  html += "var _sv = {";
  /* NTC */
  html += "ntcPin:"        + String(sen->ntc.analogPin)         + ",";
  html += "seriesR:"       + String(sen->ntc.seriesResistor)    + ",";
  html += "nomR:"          + String(sen->ntc.nominalResistance) + ",";
  html += "nomT:"          + String(sen->ntc.nominalTemp)       + ",";
  html += "bCoeff:"        + String(sen->ntc.bCoefficient)      + ",";
  html += "ntcUnit:'"      + String(sen->ntc.unit)              + "',";
  /* Analog */
  html += "analogPin:"     + String(sen->analog.analogPin)  + ",";
  html += "inputMin:"      + String(sen->analog.inputMin)   + ",";
  html += "inputMax:"      + String(sen->analog.inputMax)   + ",";
  html += "scaleMin:"      + String(sen->analog.scaleMin)   + ",";
  html += "scaleMax:"      + String(sen->analog.scaleMax)   + ",";
  html += "scaleUnit:'"    + String(sen->analog.scaleUnit)  + "',";
  /* Digital */
  html += "digitalPin:"    + String(sen->digital.digitalPin)                    + ",";
  html += "inverted:"      + String(sen->digital.inverted ? "true" : "false")   + ",";
  /* Counter */
  html += "upPin:"         + String(sen->counter.upPin)                         + ",";
  html += "downPin:"       + String(sen->counter.downPin)                       + ",";
  html += "resetPin:"      + String(sen->counter.resetPin)                      + ",";
  html += "persistent:"    + String(sen->counter.persistent ? "true" : "false") + ",";
  /* Module */
  html += "i2cAddr:"       + String(sen->module.i2cAddress)   + ",";
  html += "regIdx:"        + String(sen->module.registerIndex) + ",";
  html += "dataType:"      + String(sen->module.dataType)     + ",";
  html += "scale:"         + String(sen->module.scale)        + ",";
  html += "modUnit:'"      + String(sen->module.scaleUnit)    + "',";
  /* Publish policy */
  html += "pubTrigger:"    + String(sen->publishPolicy.trigger)         + ",";
  html += "changeThresh:"  + String(sen->publishPolicy.changeThreshold) + ",";
  html += "intervalMs:"    + String(sen->publishPolicy.intervalMs)      + "";
  html += "};";

  /* ── Central toggle function (mirrors toggleActFields) ── */
  html += "function toggleSensorFields() {";
  html += "  const typeEl = document.getElementById('sensorType');";
  html += "  const type = typeEl ? typeEl.value.trim().toLowerCase() : '" + savedType + "';";
  html += "  const container = document.getElementById('dynamicSensorFields');";
  html += "  if (!type) { container.style.display = 'none'; container.innerHTML = ''; return; }";
  html += "  container.style.display = 'block';";
  html += "  let h = '';";

  /* -------- NTC -------- */
  html += "  if (type === 'ntc') {";
  html += "    h += `<div class='form-group'><label>Analog Pin</label><select name='analogPin' required>${getPinOpts(_sv.ntcPin)}</select></div>`;";
  html += "    h += \"<div class='form-group'><label>Thermistor Preset</label><select id='ntcPreset' name='ntcPreset' onchange='toggleNtcCustom()'>\""
          "       + \"<option value='ntc10k_3435'>10k NTC B=3435</option>\""
          "       + \"<option value='ntc10k_3950'>10k NTC B=3950</option>\""
          "       + \"<option value='ntc100k_3950'>100k NTC B=3950</option>\""
          "       + \"<option value='custom'>Custom Value...</option>\""
          "       + \"</select></div>\";";
  html += "    h += \"<div id='ntcCustom' style='display:none;border-top:1px dashed #ccc;padding-top:10px'>\""
          "       + \"<div class='form-group'><label>Series Resistor (Ω)</label><input type='number' name='seriesResistor' value='\" + _sv.seriesR + \"'></div>\""
          "       + \"<div class='form-group'><label>Nominal Resistance (Ω)</label><input type='number' name='nominalResistance' value='\" + _sv.nomR + \"'></div>\""
          "       + \"<div class='form-group'><label>Nominal Temp (°C)</label><input type='number' name='nominalTemp' value='\" + _sv.nomT + \"'></div>\""
          "       + \"<div class='form-group'><label>B Coefficient</label><input type='number' name='bCoefficient' value='\" + _sv.bCoeff + \"'></div>\""
          "       + \"</div>\";";
  html += "    h += `<div class='form-group'><label>Unit</label><input type='text' name='unit' value='${_sv.ntcUnit}' required></div>`;";
  //html += "    h += \"<div class='form-group'><label>Publish only changes</label><input type='checkbox' name='publishOnlyChange'></div>\";";

  /* -------- Analog -------- */
  html += "  } else if (type === 'analog') {";
  html += "    h += `<div class='form-group'><label>Analog Pin</label><select name='analogPin' required>${getPinOpts(_sv.analogPin)}</select></div>`;";
  html += "    h += `<div class='grid'>`"
          "       + `<div><label>Input Min</label><input type='number' step='any' name='inputMin' value='${_sv.inputMin}' required></div>`"
          "       + `<div><label>Input Max</label><input type='number' step='any' name='inputMax' value='${_sv.inputMax}' required></div>`"
          "       + `</div>`;";
  html += "    h += `<div class='grid'>`"
          "       + `<div><label>Scale Min</label><input type='number' step='any' name='scaleMin' value='${_sv.scaleMin}' required></div>`"
          "       + `<div><label>Scale Max</label><input type='number' step='any' name='scaleMax' value='${_sv.scaleMax}' required></div>`"
          "       + `</div>`;";
  html += "    h += `<div class='form-group'><label>Unit</label><input type='text' name='scaleUnit' value='${_sv.scaleUnit}' placeholder='%' required></div>`;";
  //html += "    h += \"<div class='form-group'><label>Publish only changes</label><input type='checkbox' name='publishOnlyChange'></div>\";";

  /* -------- Digital -------- */
  html += "  } else if (type === 'digital') {";
  html += "    h += `<div class='form-group'><label>Digital Pin</label><select name='digitalPin' required>${getPinOpts(_sv.digitalPin)}</select></div>`;";
  html += "    h += `<label><input type='checkbox' name='inverted' ${_sv.inverted ? 'checked' : ''}> Inverted Logic</label>`;";
  //html += "    h += \"<div class='form-group'><label>Publish only changes</label><input type='checkbox' name='publishOnlyChange'></div>\";";

  /* -------- Counter -------- */
  html += "  } else if (type === 'counter') {";
  html += "    h += `<div class='form-group'><label>Up Pin</label><select name='upPin' required>${getPinOpts(_sv.upPin)}</select></div>`;";
  html += "    h += `<div class='form-group'><label>Down Pin (optional)</label><select name='downPin'><option value='-1'>None</option>${getPinOpts(_sv.downPin)}</select></div>`;";
  html += "    h += `<div class='form-group'><label>Reset Pin (optional)</label><select name='resetPin'><option value='-1'>None</option>${getPinOpts(_sv.resetPin)}</select></div>`;";
  html += "    h += `<label><input type='checkbox' name='persistent' ${_sv.persistent ? 'checked' : ''}> Persistent <small>Keep value after reboot</small></label>`;";
  //html += "    h += \"<div class='form-group'><label>Publish only changes</label><input type='checkbox' name='publishOnlyChange'></div>\";";

  /* -------- Grefur Module -------- */
  html += "  } else if (type.includes('module')) {";
  html += "    if (!sensorModules.length) {";
  html += "      h += \"<div style='color:red'>No modules discovered</div>\";";
  html += "    } else {";
  html += "      const counts = {};";
  html += "      sensorModules.forEach(m => { counts[m.name] = (counts[m.name]||0)+1; });";
  html += "      h += \"<div class='form-group'><label>Module Register</label><select id='sensorModuleSelect'>\";";
  html += "      sensorModules.forEach((m,i) => {";
  html += "        const name   = m.name.charAt(0).toUpperCase()+m.name.slice(1);";
  html += "        const suffix = counts[m.name]>1 ? ` (0x${m.addr.toString(16)})` : '';";
  html += "        const sel    = (m.addr===_sv.i2cAddr && m.reg===_sv.regIdx) ? 'selected' : '';";
  html += "        const isCurrent = (m.addr===_sv.i2cAddr && m.reg===_sv.regIdx);";
  html += "        const dis = (m.occ && !isCurrent) ? 'disabled' : '';";
  html += "        const label = (m.occ && !isCurrent) ? `${name}${suffix} (In Use)` : `${name}${suffix}`;";
  html += "        h += `<option value='${i}' ${sel} ${dis}>${label}</option>`;";
  html += "      });";
  html += "      h += '</select></div>';";
  html += "    }";
  html += "    h += `<input type='hidden' name='i2cAddress'    value='${_sv.i2cAddr}'>`;";
  html += "    h += `<input type='hidden' name='registerIndex' value='${_sv.regIdx}'>`;";
  html += "    h += `<input type='hidden' name='dataType'      value='${_sv.dataType}'>`;";
  html += "    h += `<div class='form-group'><label>Scale Multiplier</label><input type='number' step='any' name='scale' value='${_sv.scale}'></div>`;";
  html += "    h += `<div class='form-group'><label>Unit</label><input type='text' name='scaleUnit' value='${_sv.modUnit}'></div>`;";
  //html += "    h += \"<div class='form-group'><label>Publish only changes</label><input type='checkbox' name='publishOnlyChange'></div>\";";
  html += "  }";

  /* ── Shared publish policy fields (all types) ── */
  html += "  h += \"<div class='form-group'><label>Publish Trigger</label>\""
          "     + \"<select name='publishTrigger'>\""
          "     + `<option value='0' ${_sv.pubTrigger==0?'selected':''}>Threshold or Interval</option>`"
          "     + `<option value='1' ${_sv.pubTrigger==1?'selected':''}>Changes only</option>`"
          "     + `<option value='2' ${_sv.pubTrigger==2?'selected':''}>Interval only</option>`"
          "     + \"</select></div>\";";
  html += "  h += `<div class='form-group'><label>Change Threshold</label><input type='number' step='0.01' name='changeThreshold' value='${_sv.changeThresh}'></div>`;";
  html += "  h += `<div class='form-group'><label>Interval (ms)</label><input type='number' name='intervalMs' value='${_sv.intervalMs}'></div>`;";

  /* ── Write to DOM ── */
  html += "  container.innerHTML = h;";

  /* ── Bind module select → hidden fields ── */
  html += "  const modSel = document.getElementById('sensorModuleSelect');";
  html += "  if (modSel) {";
  html += "    const applyMod = () => {";
  html += "      const m = sensorModules[parseInt(modSel.value,10)];";
  html += "      if (!m) return;";
  html += "      container.querySelector('[name=i2cAddress]').value    = m.addr;";
  html += "      container.querySelector('[name=registerIndex]').value = m.reg;";
  html += "      container.querySelector('[name=dataType]').value      = m.type;";
  html += "    };";
  html += "    modSel.onchange = applyMod;";
  html += "    applyMod();";
  html += "  }";

  /* ── NTC custom fields toggle ── */
  html += "  window.toggleNtcCustom = function() {";
  html += "    const preset = document.getElementById('ntcPreset');";
  html += "    const el     = document.getElementById('ntcCustom');";
  html += "    if (preset && el) el.style.display = (preset.value==='custom') ? 'block' : 'none';";
  html += "  };";
  html += "  const ntcPreset = document.getElementById('ntcPreset');";
  html += "  if (ntcPreset) toggleNtcCustom();";

  html += "}"; /* end toggleSensorFields */

  /* ── Initial call ── */
  html += "toggleSensorFields();";

  /* ── Wire up the type dropdown if present ── */
  html += "var _typeEl = document.getElementById('sensorType');";
  html += "if (_typeEl) _typeEl.addEventListener('change', toggleSensorFields);";
...
/* ============================================================
   DisplayAddSensorFields()  – simplified (mirrors DisplayAddActuatorFields)
   ============================================================ */
String DisplayAddSensorFields() {
  String html = "<div class='sensor-form-section' style='margin-top:2vh;'>";
  html += "<h2 id='addNewSensor'>Add New Sensor</h2>";

  int configuredCount = sensorManager.getConfiguredSensorCount();

  /* Resource manager slot indicator */
  html += getGrefurResourceManager("sensor", configuredCount, MAX_SENSORS_ALLOWED, "Sensor");

  /* Form visibility mirrors actuator pattern */
  html += "<form id='sensorForm' action='/config/sensor' method='POST' style='display:"
          + String(configuredCount < MAX_SENSORS_ALLOWED ? "flex" : "none")
          + "; flex-direction:column; gap:15px;'>";

  html += getSensorFormFields(); /* No config pointer → Add mode */

  html += "  <input type='button' id='addSensorBtn' class='inheritSubmitStyle' value='Add Sensor' style='margin-top:10px;'>";
  html += "  <div id='responseMessage' style='min-height:24px; margin-top:10px;'></div>";
  html += "</form>";
  html += "</div>";
  return html;
}
...
/* ============================================================
   handleGetEditPage()  – simplified (mirrors handleGetEditActuatorPage)
   ============================================================ */
void handleGetEditPage() {
  ...
  html.push_back(getHTMLHeader("Sensor Edit"));
  html.push_back(mainBody("start"));
  html.push_back("<h1>Edit Sensor: " + String(sensor.name) + "</h1>");
  html.push_back(getSimpleDeviceInfoHTML(
                   (lastMeasurement != "nan" && lastMeasurement != "") ? lastMeasurement + " " + unit : ""
                 ));

  html.push_back("<form id='sensorForm' action='/config/sensor' method='POST' style='display:flex; flex-direction:column; gap:15px;'>");
  html.push_back("  <input type='hidden' name='sensorId' value='" + String(sensorId) + "'>");
  html.push_back("  <input type='hidden' name='name'     value='" + String(sensor.name) + "'>");
  html.push_back("  <input type='hidden' name='type'     value='" + String(sensor.type) + "'>");

  html.push_back(getSensorFormFields(&sensor)); /* Config pointer → Edit mode, pre-populated */

  html.push_back("  <div style='display:flex; gap:10px; margin-top:10px;'>");
  html.push_back("    <button type='submit' class='inheritSubmitStyle' style='flex:1;'>Save &amp; Exit</button>");
  html.push_back("    <button type='button' onclick='SaveSettingsInline()' class='inheritSubmitStyle' style='flex:1; background-color:var(--secondary);'>Save Changes</button>");
  html.push_back("  </div>");
  html.push_back("</form>");

  html.push_back("<script>");
  html.push_back("async function SaveSettingsInline() {");
  html.push_back("  const form = document.getElementById('sensorForm');");
  html.push_back("  const formData = new FormData(form);");
  html.push_back("  try {");
  html.push_back("    const response = await fetch(form.action, { method: 'POST', body: formData });");
  html.push_back("    if (response.ok) { alert('Changes saved successfully!'); }");
  html.push_back("    else             { alert('Save failed.'); }");
  html.push_back("  } catch (err) { console.error(err); alert('Error connecting to server'); }");
  html.push_back("}");
  html.push_back("</script>");
...
/* ============================================================
   SensorTableScript()  – unchanged from original
   (kept here for completeness; no structural changes needed)
   ============================================================ */
String SensorTableScript() {
  String html = "<script id='updateSensorTableScript'>";

  html += "async function refreshDynamicSection(selector) {";
  html += "  const response = await fetch('/', { cache: 'no-store' });";
  html += "  if (!response.ok) throw new Error('Refresh failed: ' + response.status);";
  html += "  const text = await response.text();";
  html += "  const doc = new DOMParser().parseFromString(text, 'text/html');";
  html += "  const fresh = doc.querySelector(selector);";
  html += "  const current = document.querySelector(selector);";
  html += "  if (!fresh || !current) return;";
  html += "  current.replaceWith(fresh);";
  html += "  fresh.querySelectorAll('script').forEach(oldScript => {";
  html += "    const script = document.createElement('script');";
  html += "    script.text = oldScript.textContent;";
  ...
  html += "    if (oldTable && newTable) {";
  html += "      oldTable.replaceWith(newTable);";
  html += "      await refreshDynamicSection('.sensor-form-section');";
  html += "      const rows = newTable.querySelectorAll('tbody tr').length;";
  html += "      const isEmpty = newTable.innerHTML.includes('No sensors') || newTable.innerHTML.includes('No items');";
  html += "      const actualCount = (rows === 1 && isEmpty) ? 0 : rows;";
  html += "      if (typeof syncGrefurResource === 'function') {";
  html += "        console.log('actualCount:', actualCount, 'rows:', rows, 'isEmpty:', isEmpty);";
  html += "        const isAvailable = syncGrefurResource('sensor', actualCount, 'Sensor');";
  html += "        console.log('isAvailable:', isAvailable);";
  html += "        const form = document.getElementById('sensorForm');";
  html += "        if (form) {";
  html += "          form.style.display = isAvailable ? 'flex' : 'none';";
  html += "        }";
  html += "      }";
  html += "    }";
  html += "  } catch (err) { console.error('Sensor AJAX error:', err); }";
  html += "}";

  html += "document.addEventListener('click', async function(e) {";
  html += "  if (e.target && e.target.id === 'addSensorBtn') {";
  html += "    const form = document.getElementById('sensorForm');";
  html += "    if (!form.querySelector('[name=name]').value.trim() || !form.querySelector('[name=type]').value.trim()) {";
  html += "      alert('Please fill in Sensor Name and Type');";
  html += "      return;";
  html += "    }";
  html += "    const btn = e.target;";
  html += "    btn.disabled = true;";
  html += "    const formData = new FormData(form);";
  html += "    await updateSensorTable(form.action + '?responseFormat=html', { method: form.method, body: formData });";
  html += "    form.reset();";
  html += "    const dyn = document.getElementById('dynamicSensorFields');";
  html += "    if (dyn) dyn.innerHTML = '';";
  html += "    btn.disabled = false;";
  html += "    if (typeof toggleSensorFields === 'function') toggleSensorFields();";
  html += "  }";
  html += "});";

  html += "function deleteSensor(id) {";
  html += "  if (confirm('Delete sensor ' + id + '?')) {";
...
/* {Summary: Generates the HTML table for configured sensors including Grefur Modules} */
String DisplaySensorsInProduction() {
  ...
  for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
    ...
    else if (config.sensors[i].type == SENSOR_COUNTER) typeName = "Counter";
    else if (config.sensors[i].type == SENSOR_MODULE_VALUE) typeName = "Grefur Module";
    ...
    else if (config.sensors[i].type == SENSOR_COUNTER) {
      details = "Up=GPIO" + String(config.sensors[i].counter.upPin) +
                (config.sensors[i].counter.downPin >= 0 ? " Down=GPIO" + String(config.sensors[i].counter.downPin) : "") +
                (config.sensors[i].counter.resetPin >= 0 ? " Rst=GPIO" + String(config.sensors[i].counter.resetPin) : "") +
                (config.sensors[i].counter.persistent ? " [persistent]" : "") + " " +
                publishMode;
    }
    else if (config.sensors[i].type == SENSOR_MODULE_VALUE) {
      // Readable format for data type
      String dtName;
      switch (config.sensors[i].module.dataType) {
        case 0: dtName = "U8"; break;
        case 1: dtName = "U16"; break;
        case 2: dtName = "I16"; break;
        case 3: dtName = "I32"; break;
        case 4: dtName = "F32"; break;
        default: dtName = "RAW"; break;
      }

      details = "I2C Addr: 0x" + String(config.sensors[i].module.i2cAddress, HEX) +
                ", Reg: " + String(config.sensors[i].module.registerIndex) +
                " (" + dtName + "), " +
                "Scale: x" + String(config.sensors[i].module.scale) + " " +
                String(config.sensors[i].module.scaleUnit) + " " +
                publishMode;
    }
  ...
  if (!hasConfiguredSensors) {
    html += "<tr><td colspan='5' style='text-align:center;'>No sensors configured yet.</td></tr>";
  }
...
String calculationTableScript() {
  ...
  // Ensure publishValue is sent as 0 when the checkbox is not checked.
...
String displayAddCalculationFields() {
  ...
  if (availableSlots <= 0) {
    html += DisplaySellUpgrades("Actuators");
    html += "</div>";
    return html;
  }
...
/* {Summary: AJAX handler for Actuator updates. Replaces the table in the DOM and triggers the resource sync engine.} */
String actuatorTableScript() {
  ...
  /* Update logic nested within brackets to ensure correct execution order */
  html += "    if (oldTable && newTable) {";
  html += "      oldTable.replaceWith(newTable);";
  html += "      await refreshDynamicSection('.actuator-form-section');";

  /* Count actual rows in the new table */
  html += "      const rows = newTable.querySelectorAll('tbody tr').length;";
  html += "      const isEmpty = newTable.innerHTML.includes('No actuators') || newTable.innerHTML.includes('No items');";
  html += "      const actualCount = (rows === 1 && isEmpty) ? 0 : rows;";

  /* Call the sync engine with matching prefix and label */
  html += "      if(typeof syncGrefurResource === 'function') {";
  html += "        syncGrefurResource('actuator', actualCount, 'Actuator');";
  html += "      }";
  html += "    } else {";
  html += "      console.warn('Target table not found in response');";
  html += "    }";

  html += "  } catch (err) { ";
  html += "    console.error('AJAX sync failed:', err);";
  html += "    alert('Update failed: ' + err.message); ";
  html += "  }";
  html += "}";

  /* Logic for Add Actuator click events */
  html += "document.addEventListener('click', async function(e) {";
  html += "  if (e.target && e.target.id === 'addActBtn') {";
  html += "    const form = document.getElementById('actuatorForm');";

  /* Field validation */
  html += "    if (!form.querySelector('[name=name]').value.trim() || !form.querySelector('[name=sourceName]').value.trim()) {";
  html += "      alert('Required fields missing');";
  html += "      return;";
  html += "    }";
  ...
  /* Convert checkboxes to explicit 1/0 for the backend */
  html += "    const inv = form.querySelector('[name=inverted]');";
  html += "    if(inv) formData.set('inverted', inv.checked ? '1' : '0');";
  ...
  /* UI Reset */
  html += "    form.reset();";
  html += "    const dyn = document.getElementById('dynamicActFields');";
  html += "    if (dyn) dyn.innerHTML = '';";
  html += "    btn.disabled = false;";
  ...
  /* Global function for deleting actuators */
  html += "function deleteActuator(id) {";
  html += "  if (confirm('Delete actuator ' + id + '?')) {";
...
/* {Summary: Displays configured actuator slots in a responsive table, including Grefur Module outputs} */
String displayActuatorsInProduction() {
  String html = "";
  bool hasConfiguredActuators = false;

  html += "<div class='sensor-list' id='actuator-list'>";
  html += "<h2 id='acturatorHeader'>Configured Actuators</h2>";
  html += "<table class='sensor-table' id='actuator-table'>";
  html += "<thead><tr>";
  html += "<th>ID</th>";
  html += "<th>Name</th>";
  html += "<th>Mirror Source</th>";
  html += "<th>Hardware Details</th>";
  html += "<th>Actions</th>";
  html += "</tr></thead>";
  html += "<tbody>";
  ...
  for (int i = 0; i < MAX_ACTUATORS_ALLOWED; i++) {
    ...
    // Generate Hardware Details string based on type
    String details = "";
    if (act.type == ACTUATOR_DIGITAL) {
      details = "Digital: GPIO" + String(act.digital.digitalPin) +
                (act.digital.inverted ? " (Inverted)" : "");
    }
    else if (act.type == ACTUATOR_ANALOG) {
      details = "Analog: GPIO" + String(act.analog.analogPin) +
                ", Range: " + String(act.analog.minValue) + "-" + String(act.analog.maxValue) +
                " " + String(act.analog.unit);
    }
    else if (act.type == ACTUATOR_MODULE_VALUE) { // Assuming type 2
      String dtName;
      switch (act.module.dataType) {
        case 1: dtName = "U8"; break;
        case 2: dtName = "I16"; break;
        case 3: dtName = "U16"; break;
        case 4: dtName = "I32"; break;
        case 5: dtName = "F32"; break;
        default: dtName = "RAW"; break;
      }

      details = "Grefur Module: 0x" + String(act.module.i2cAddress, HEX) +
                ", Reg: " + String(act.module.registerAddress) +
                " (" + dtName + "), Scale: x" + String(act.module.scale);

      if (act.module.offset != 0.0f) {
        details += ", Offset: " + String(act.module.offset);
      }
    }

    html += "<td><p class='sensorHeaderSmallScreen'>Hardware</p>" + details + "</td>";
  ...
  if (!hasConfiguredActuators) {
    html += "<tr><td colspan='5' style='text-align:center;'>No actuators configured yet.</td></tr>";
  }
...
/* {Summary: Generates Actuator form with dynamic GPIO dropdowns and auto-discovery for Grefur module outputs.
   Includes occupancy detection logic to prevent resource collisions while allowing editing of current assignments.} */
String getActuatorFormFields(const ActuatorConfig* act = nullptr) {
  int currentActId = -1;

  /* Identify which actuator index we are editing to prevent self-blocking in occupancy checks */
  if (act != nullptr) {
    for (int i = 0; i < MAX_ACTUATORS_ALLOWED; i++) {
      if (act == &config.actuators[i]) {
        currentActId = i;
        break;
      }
    }
  ...
  /* Create temporary default config if act is null (New Actuator) */
  ActuatorConfig d;
  if (!act) {
  ...
  html += "<option value='0' " + String(act->type == 0 ? "selected" : "") + ">Digital (GPIO High/Low)</option>";
  html += "<option value='1' " + String(act->type == 1 ? "selected" : "") + ">Analog (GPIO PWM/DAC)</option>";
  html += "<option value='2' " + String(act->type == 2 ? "selected" : "") + ">Grefur Module Output</option>";
  html += "</select></div>";

  html += "<div id='dynamicActFields'></div>";

  html += "<script>";

  /* {Summary: Filter GPIO pins - Only include available pins or the one currently assigned to this actuator} */
  html += "var availablePins = [";
  int pins[] = {0, 1, 2, 3, 4, 5, 21, 43, 44};
  bool first = true;
  for (int i = 0; i < 9; i++) {
    bool occ = isResourceOccupied(pins[i], 0, 0, currentActId, true);
    bool isCurrent = false;
    if (act->type == 0 && pins[i] == act->digital.digitalPin) isCurrent = true;
    if (act->type == 1 && pins[i] == act->analog.analogPin) isCurrent = true;
  ...
  /* {Summary: Generate module JSON with an 'occ' flag. currentActId ensures the current actuator's own registers aren't flagged} */
  html += "var grefurModules = ";
  String moduleDataJson = "[";
  for (uint8_t d = 0; d < ModuleEngine.getDeviceCount(); d++) {
    for (uint8_t r = 0; r < ModuleEngine.getRegisterCount(d); r++) {
      if (!ModuleEngine.getDevice(d)->registers[r].writable) continue;

      uint8_t addr = ModuleEngine.getDeviceAddress(d);
      uint16_t reg = ModuleEngine.getRegisterAddress(d, r);
      bool occ = isResourceOccupied(-1, addr, reg, currentActId, true);

      if (moduleDataJson.length() > 1) moduleDataJson += ",";
      moduleDataJson += "{\"name\":\"" + String(ModuleEngine.getRegisterName(d, r)) + "\",";
      moduleDataJson += "\"addr\":" + String(addr) + ",\"reg\":" + String(reg) + ",";
      moduleDataJson += "\"type\":" + String(ModuleEngine.getRegisterDataType(d, r)) + ",";
      moduleDataJson += "\"occ\":" + String(occ ? "true" : "false") + "}";
    }
  }
  moduleDataJson += "]";
  html += moduleDataJson + ";";

  /* Logic to switch field sets based on actuator type */
  html += "function toggleActFields() {";
  html += "  const type = document.getElementById('actType').value;";
  html += "  const container = document.getElementById('dynamicActFields');";
  html += "  let h = '';";

  /* Digital GPIO Fields */
  html += "  if (type === '0') {";
  html += "    h += \"<div class='form-group'><label>Digital Pin</label><select name='digitalPin'>\";";
  html += "    availablePins.forEach(p => {";
  html += "      const sel = (p == " + String(act->type == 0 ? act->digital.digitalPin : 0) + ") ? 'selected' : '';";
  html += "      h += `<option value='${p}' ${sel}>GPIO ${p}</option>`;";
  html += "    });";
  html += "    h += \"</select></div>\";";
  html += "    h += \"<label><input type='checkbox' name='inverted' value='1' " + String((act->type == 0 && act->digital.inverted) ? "checked" : "") + "> Inverted Logic</label>\";";

  /* Analog GPIO Fields */
  html += "  } else if (type === '1') {";
  html += "    h += \"<div class='grid'><div><label>Analog Pin</label><select name='analogPin'>\";";
  html += "    availablePins.forEach(p => {";
  html += "      const sel = (p == " + String(act->type == 1 ? act->analog.analogPin : 0) + ") ? 'selected' : '';";
  html += "      h += `<option value='${p}' ${sel}>GPIO ${p}</option>`;";
  html += "    });";
  html += "    h += \"</select></div>\";";
  html += "    h += \"<div><label>Unit</label><input type='text' name='unit' value='" + String(act->type == 1 ? act->analog.unit : "%") + "'></div></div>\";";
  html += "    h += \"<div class='grid'><div><label>Logic Min</label><input type='number' step='0.1' name='minValue' value='" + String(act->type == 1 ? act->analog.minValue : 0) + "'></div>\";";
  html += "    h += \"<div><label>Logic Max</label><input type='number' step='0.1' name='maxValue' value='" + String(act->type == 1 ? act->analog.maxValue : 100) + "'></div></div>\";";

  /* Grefur Module Fields with Duplicate Name Handling */
  html += "  } else if (type === '2') {";
  html += "    h += \"<div class='form-group'><label>Select Module Output</label><select id='moduleSelect' class='form-control'>\";";
  html += "    h += \"<option value='-1'>Select Output</option>\";";

  String curAddr = String(act->type == 2 ? act->module.i2cAddress : 0);
  String curReg = String(act->type == 2 ? act->module.registerAddress : 0);

  html += "    const counts = {}; grefurModules.forEach(m => { counts[m.name] = (counts[m.name] || 0) + 1; });";
  html += "    grefurModules.forEach((m, idx) => {";
  html += "      const name = m.name.charAt(0).toUpperCase() + m.name.slice(1);";
  html += "      const display = counts[m.name] > 1 ? `${name} (0x${m.addr.toString(16).toUpperCase()})` : name;";

  /* Use strict numeric comparison for current selection */
  html += "      const isCurrent = (Number(m.addr) === Number(" + curAddr + ") && Number(m.reg) === Number(" + curReg + "));";
  html += "      const dis = (m.occ && !isCurrent) ? 'disabled' : '';";
  html += "      const label = (m.occ && !isCurrent) ? `${display} (In Use)` : display;";
  html += "      const sel = isCurrent ? 'selected' : '';";
  html += "      h += `<option value='${idx}' ${sel} ${dis}>${label}</option>`;";
  html += "    });";
  html += "    h += \"</select></div>\";";

  /* Hidden fields for I2C metadata */
  html += "    h += \"<input type='hidden' id='hiddenAddr' name='i2cAddress' value='" + curAddr + "'>\";";
  html += "    h += \"<input type='hidden' id='hiddenReg' name='registerAddress' value='" + curReg + "'>\";";
  html += "    h += \"<input type='hidden' id='hiddenType' name='dataType' value='" + String(act->type == 2 ? act->module.dataType : 0) + "'>\";";
  html += "    h += \"<div class='grid'><div><label>Scale</label><input type='number' step='any' name='scale' value='" + String(act->type == 2 ? act->module.scale : 1.0) + "'></div>\";";
  html += "    h += \"<div><label>Offset</label><input type='number' step='any' name='offset' value='" + String(act->type == 2 ? act->module.offset : 0.0) + "'></div></div>\";";
  html += "  }";

  html += "  container.innerHTML = h;";

  /* Bind I2C metadata to hidden fields on change */
  html += "  const selEl = document.getElementById('moduleSelect');";
  html += "  if(selEl) {";
  html += "    selEl.onchange = function() {";
  html += "      const s = grefurModules[this.value];";
  html += "      if(s) {";
  html += "        document.getElementById('hiddenAddr').value = s.addr;";
  html += "        document.getElementById('hiddenReg').value = s.reg;";
  html += "        document.getElementById('hiddenType').value = s.type;";
  html += "      }";
  html += "    };";
  html += "  }";
  html += "}";
...
/* {Summary: Updated Add Actuator section. The form ID is now the direct target for the sync logic.} */
String DisplayAddActuatorFields() {
  ...
  /* Prefix is 'actuator' so the script looks for 'actuatorForm' */
  html += getGrefurResourceManager("actuator", currentCount, MAX_ACTUATORS_ALLOWED, "Actuator");

  /* The form itself is now the visibility target */
  html += "<form id='actuatorForm' action='/config/actuator' method='POST' style='display:" + String(currentCount < MAX_ACTUATORS_ALLOWED ? "flex" : "none") + "; flex-direction:column; gap:15px;'>";

  html += "  <input type='hidden' name='actId' value=''>";
  html +=    getActuatorFormFields();
  html += "  <button type='button' id='addActBtn' class='inheritSubmitStyle' style='margin-top:10px;'>Add Actuator</button>";
  html += "  <div id='responseMessage' style='min-height: 24px; margin-top: 10px;'></div>";
...
void handleGetEditActuatorPage() {
  ...
  if (!server.hasArg("id")) {
    asyncResponse({"Missing ID"}, false);
    return;
  }
  ...
  html.push_back(mainBody("start"));
  html.push_back("<h1>Edit Actuator: " + String(act.name) + "</h1>");
...
void handleCertificatePages() {
  ...
  html += certificateManager.getCertificateSummary();
  html += "</textarea><br><br>";
  ...
  html += "<form method='POST' action='/certificate/upload'>";
  html += "<label>Certificate PEM</label><br>";
  html += "<textarea name='cert' rows='10' style='width:100%; font-family:monospace;' required></textarea><br><br>";
  html += "<label>Private Key PEM</label><br>";
  html += "<textarea name='key' rows='10' style='width:100%; font-family:monospace;' required></textarea><br><br>";
  html += "<label>Root CA PEM (optional)</label><br>";
  html += "<textarea name='rootCA' rows='8' style='width:100%; font-family:monospace;'></textarea><br><br>";
  html += "<input type='submit' value='Save Certificate'>";
  html += "</form><br>";
...
void handleRoot() {
  DEBUG_PRINT(DEBUG, "Root page requested");
  ...
  server.sendContent(GrefurServicesScript());
...
void handleSensorTypeRequest() {
  ...
  // Handle GET parameter for sensor type lookup.
  if (server.method() == HTTP_GET) {
    if (server.hasArg("type")) {
      ...
      // Return all sensor types when no specific type is requested.
...
void handleSensorConfig() {
  ...
  if (typeStr == "ntc" || typeStr == "0") {
  ...
  else if (typeStr == "counter" || typeStr == "3") {
    newConfig.type = SENSOR_COUNTER;
    newConfig.counter.upPin    = server.arg("upPin").toInt();
    newConfig.counter.downPin  = server.hasArg("downPin")  ? server.arg("downPin").toInt()  : -1;
    newConfig.counter.resetPin = server.hasArg("resetPin") ? server.arg("resetPin").toInt() : -1;
    newConfig.counter.persistent = server.arg("persistent") == "on";

    // Counter uses the same publish setup as digital sensors; threshold is ignored.
    uint8_t triggerVal = (uint8_t)server.arg("publishTrigger").toInt();
    newConfig.publishPolicy.trigger = (triggerVal == PUBLISH_ON_INTERVAL)
                                      ? PUBLISH_ON_INTERVAL
                                      : PUBLISH_ON_THRESHOLD_OR_INTERVAL;
    ...
    // Reset _reserved so the slot starts from a clean state.
    memset(newConfig._reserved, 0, sizeof(newConfig._reserved));

    Serial.println("Counter Config:");
    Serial.println("  upPin: "     + String(newConfig.counter.upPin));
    Serial.println("  downPin: "   + String(newConfig.counter.downPin));
    Serial.println("  resetPin: "  + String(newConfig.counter.resetPin));
    Serial.println("  persistent: " + String(newConfig.counter.persistent ? "true" : "false"));
  }
  /* Handling the Grefur Module with 16-bit register support */
  else if (typeStr == "module" || typeStr == "4" || typeStr == "grefur module") {
    newConfig.type = SENSOR_MODULE_VALUE;

    newConfig.module.i2cAddress    = (uint8_t)server.arg("i2cAddress").toInt();

    // registerIndex is uint16_t, so values above 255 are safe.
    newConfig.module.registerIndex = (uint16_t)server.arg("registerIndex").toInt();

    if (isResourceOccupied(-1, newConfig.module.i2cAddress, newConfig.module.registerIndex, isEdit ? existingId : -1, false)) {
      server.send(409, "text/plain", "Module input is already in use");
      return;
    }

    newConfig.module.dataType      = (int8_t)server.arg("dataType").toInt();
    newConfig.module.scale         = server.arg("scale").toFloat();
    newConfig.module.offset        = server.arg("offset").toFloat();
    newConfig.module.timeoutMs     = (uint16_t)server.arg("timeoutMs").toInt();
    newConfig.module.errorCount    = 0; // Runtime reset

    // Copy unit, max 7 characters plus null terminator.
    String scaleUnitStr = server.arg("scaleUnit");
    strncpy(newConfig.module.scaleUnit, scaleUnitStr.c_str(), sizeof(newConfig.module.scaleUnit) - 1);
    newConfig.module.scaleUnit[sizeof(newConfig.module.scaleUnit) - 1] = '\0';

    // Publish policy using standard Grefur threshold/interval behavior.
    uint8_t triggerVal = (uint8_t)server.arg("publishTrigger").toInt();
    newConfig.publishPolicy.trigger = (triggerVal <= 2) ? (PublishTrigger)triggerVal : PUBLISH_ON_THRESHOLD_OR_INTERVAL;
    newConfig.publishPolicy.changeThreshold = server.arg("changeThreshold").toFloat();
    newConfig.publishPolicy.intervalMs = server.arg("intervalMs").toInt();

    Serial.println("Module Config (16-bit Reg): Address " + String(newConfig.module.i2cAddress) +
                   " Index " + String(newConfig.module.registerIndex));
  }
  else {
  ...
  if (isEdit) {
    ...
    char oldName[16];
    strncpy(oldName, config.sensors[existingId].name, sizeof(oldName) - 1);
    oldName[sizeof(oldName) - 1] = '\0';

    auto result = sensorManager.updateSensor(existingId, newConfig);
    sensorId = result.sensorId;

    if (result.unitChanged) {
      mqtt.clearBlockedSensor(oldName);  // ← use old name, that's what's in the block list
    }
  } else {
...
/*
   Endpoint for calculation requests
    -GET
    -ADD / POST
    -DELETE
*/
...
void handleCalculationSave() {
  ...
  switch (newCalc.type) {
    case CALC_EXPRESSION:
      if (!expressionManager.validateExpression(expression, error)) {
        server.send(400, "text/plain", "Input Error: " + error);
        return;
      }
  ...
  if (server.hasArg("calcId") && server.arg("calcId") != "") {
    ...
    if (id >= 0 && id < MAX_CALCULATIONS_ALLOWED) {
      ...
      char oldName[16];
      char oldUnit[8];
      strncpy(oldName, config.calculations[id].name, sizeof(oldName) - 1);
      strncpy(oldUnit, config.calculations[id].unit, sizeof(oldUnit) - 1);
      oldName[sizeof(oldName) - 1] = '\0';
      oldUnit[sizeof(oldUnit) - 1] = '\0';

      expressionManager.updateCalculation(id, newCalc, mqtt);

      // If unit changed, unblock so next publish sends fresh metadata
      if (strncmp(oldUnit, newCalc.unit, sizeof(oldUnit)) != 0) {
        mqtt.clearBlockedSensor(oldName);
      }
...
void handleActuatorSave() {
  ...
  newAct.publishPolicy.trigger = (triggerVal <= PUBLISH_ON_INTERVAL)
                                 ? (PublishTrigger)triggerVal
                                 : PUBLISH_ON_THRESHOLD_OR_INTERVAL;
  newAct.publishPolicy.changeThreshold = server.arg("changeThreshold").toFloat();
  newAct.publishPolicy.intervalMs      = (uint32_t)server.arg("intervalMs").toInt();

  // Type-specific fields
  switch (newAct.type) {
    case ACTUATOR_DIGITAL:
      newAct.digital.digitalPin = (uint8_t)server.arg("digitalPin").toInt();
      newAct.digital.inverted   = server.hasArg("inverted") && (server.arg("inverted") == "1");
      break;
    ...
    case ACTUATOR_MODULE_VALUE:
      newAct.module.i2cAddress     = (uint8_t)server.arg("i2cAddress").toInt();
      newAct.module.registerAddress = (uint16_t)server.arg("registerAddress").toInt();
      {
        int currentActId = (server.hasArg("actId") && server.arg("actId") != "") ? server.arg("actId").toInt() : -1;
        if (isResourceOccupied(-1, newAct.module.i2cAddress, newAct.module.registerAddress, currentActId, true)) {
          server.send(409, "text/plain", "Module output is already in use");
          return;
        }
      }
      newAct.module.dataType       = (int8_t)server.arg("dataType").toInt();
      newAct.module.offset         = server.arg("offset").toFloat();
      newAct.module.timeoutMs      = 1000;

      // Guard against zero scale — would cause silent division errors in writeModule
      newAct.module.scale = (server.arg("scale").toFloat() != 0.0f)
                            ? server.arg("scale").toFloat()
                            : 1.0f;
      break;
  ...
  if (server.hasArg("actId") && server.arg("actId") != "") {
    ...
    finalId = actuatorManager.addActuator(newAct);
    success  = (finalId >= 0);
  }

  if (success) {
    // Force an immediate hardware update on next loop by invalidating the cached runtime value.
    // NAN ensures the delta check always passes regardless of target value.
    config.actuators[finalId].runtimeValue = NAN;
    ...
    if (server.arg("responseFormat") == "html") {
      ...
      server.sendHeader("Location", String("/") + "#actuatorHeader");
      server.send(303);
    }
  } else {
...
void handleWinkRequest() {
  ...
  String state = server.arg("state"); // Read ?state=on/off.
  if (state == "on") {
    digitalWrite(ONBOARD_LED, LOW);   // LED on for active-low ESP8266 targets.
    winkActive = true;
...
void handleBackupRequest() {
  ...
  String backupJson = BackupManager::generateBackupJsonBase64(config);
  if (DEBUG) {
...
void handleBackupUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    backupRestoreSucceeded = restoreSuccess;
    if (restoreSuccess) {
      Serial.println("EEPROM restored successfully from uploaded backup!");
      backupRestoreMessage = "Backup restored successfully. Device will restart in 3 seconds...";
    } else {
      Serial.println("Failed to restore EEPROM from uploaded backup.");
      backupRestoreMessage = "Failed to restore backup. Check the serial console for details.";
    }
...
/*
   GET : Returns data for all available pins and their status, "free" or "occupied".
   Params:
     pin type
      -> inputs
      -> outputs
   {"name": "free"...}
*/
/*void handleResourceStatus(AsyncWebServerRequest *request) {
...
void handleCertificateUpload() {
  ...
  if (!server.hasArg("cert") || !server.hasArg("key")) {
    server.send(400, "text/plain", "Missing certificate or key file");
    return;
  }

  String certStr = server.arg("cert");
  String keyStr = server.arg("key");
  String rootCAStr = server.hasArg("rootCA") ? server.arg("rootCA") : "";
  ...
  String result = certificateManager.saveCertificate(certStr, keyStr, rootCAStr);
...
void debugSystemStatus() {
...
#else
  Serial.println("Temperature:         not available on this target");
#endif
  ...
  float    ramUsed      = 100.0f * (ramTotal - freeHeap) / ramTotal;
  Serial.printf("RAM total:           %u bytes\n",   ramTotal);
  Serial.printf("RAM free:            %u bytes\n",   freeHeap);
  Serial.printf("RAM min free (boot): %u bytes\n",   minFreeHeap);
  Serial.printf("RAM max alloc block: %u bytes\n",   maxAlloc);
  ...
  Serial.printf("Sketch free space:   %u bytes\n",  ESP.getFreeSketchSpace());

  // ── EEPROM / NVS ────────────────────────────────────────────────
  Serial.printf("EEPROMLayout size:   %u bytes\n",  sizeof(EEPROMLayout));
  Serial.printf("  EEPROMHeader:      %u bytes\n",  sizeof(EEPROMHeader));
  Serial.printf("  DeviceMetadata:    %u bytes\n",  sizeof(DeviceMetadata));
  Serial.printf("  NetworkSettings:   %u bytes\n",  sizeof(NetworkSettings));
  Serial.printf("  MQTTConfig:        %u bytes\n",  sizeof(MQTTConfig));
  Serial.printf("  OperationSettings: %u bytes\n",  sizeof(OperationSettings));
  Serial.printf("  SensorConfig x%d:  %u bytes\n",  MAX_SENSORS_ALLOWED,      sizeof(SensorConfig)      * MAX_SENSORS_ALLOWED);
  Serial.printf("  ActuatorConfig x%d: %u bytes\n", MAX_ACTUATORS_ALLOWED,    sizeof(ActuatorConfig)    * MAX_ACTUATORS_ALLOWED);
  Serial.printf("  CalcConfig x%d:    %u bytes\n",  MAX_CALCULATIONS_ALLOWED, sizeof(CalculationConfig) * MAX_CALCULATIONS_ALLOWED);
  ...
  switch (resetReason) {
    ...
    case ESP_RST_DEEPSLEEP: resetStr = "Deep sleep wakeup"; break;
  ...
  Serial.printf("Stack high water:    %u bytes\n", uxTaskGetStackHighWaterMark(task));
  Serial.printf("FreeRTOS tasks:      %u running\n", uxTaskGetNumberOfTasks());
...
void setupServerRoutes() {
  ...
  server.on("/firmware", HTTP_POST, []() {
    if (!otaUpdateStarted || !otaUpdateSucceeded || Update.hasError()) {
      String message = otaUpdateError.length() > 0 ? otaUpdateError : Update.errorString();
      if (message.length() == 0) message = "No valid firmware upload was completed.";
      server.send(500, "text/plain", String("Update failed: ") + message);
    } else {
  ...
  server.on("/backup/file", HTTP_GET, handleBackupRequest); // Backup download endpoint
  server.on("/backup/upload", HTTP_POST, []() {
    if (backupRestoreCompleted && backupRestoreSucceeded) {
      String response = backupRestoreMessage;
      response += "<script>setTimeout(function(){window.location='/'},3000);</script>";
      server.send(200, "text/html", response);
      delay(3000);
      ESP.restart();
    } else if (backupRestoreCompleted) {
      server.send(500, "text/plain", backupRestoreMessage);
    } else {
      server.send(400, "text/plain", "No backup file was uploaded.");
    }
  }, handleBackupUpload); // Backup upload
...
void beginMicroControllerServer() {
  ...
  if (certificateManager.validateCertificate()) {
    secureServer->start();
    Serial.println("[HTTPS] Server startet på port 443");
  } else {
    Serial.println("[HTTPS] Ingen gyldige sertifikater – kan ikke starte HTTPS");
  }
#else
  server.begin();
...
void setup() {
  ...
  Serial.println("Før LED");

  pinMode(ONBOARD_LED, OUTPUT);

  if (!EEPROM.begin(sizeof(EEPROMLayout))) {
    Serial.println("CRITICAL: EEPROM.begin failed!");
    while (1) {
      delay(1000);  // Hang here so you can read the logs
    }
  }
  ...
  if (!loadConfig() || loadDefault) {
    ...
    if (!saveConfig()) {
      Serial.println("Failed to persist default config!");
      // handle error — don't reset
      
    } else {
      Serial.println("Config was saved successfully");
    }

    //visualizationOfError(GENERAL_ERROR);
  }

  sensorManager.initCounterReadings();
  expressionManager.initRetainReadings();
  ModuleEngine.begin(SDA_PIN, SCL_PIN, I2C_CLOCK);
  uint8_t found = ModuleEngine.scanAndDiscover();
  //Serial.printf("[SCAN] Done. Found %u module(s).\n", found);

  //Serial.printf("Fingerprint: %s\n", config.device.productionToken);

  BackupManager::begin(); // Load EEPROM or apply defaults.

#if SCREEN_MODULE
  displayManager.initScreen();
  // Connect the screen manager to the binding engine for value retrieval.
  displayManager.setBindEngine(BindingEngine);
#endif
  ...
  //pinMode(MUX_B, OUTPUT);
   ...
   Serial.println("Før integritetstest");

  testConfigIntegrity();

  Serial.println("Etter integritetstest");
...
void loop() {
  ...
  static unsigned long lastLoopTime = 1;
  ...
  // Bus rediscovering
  static unsigned long lastBusDiscoveryTime = 0;
  const unsigned long BUS_DISCOVERY_INTERVAL = 30000;
...
#define TICK_TIME 50
  ...
  if (currentTime - lastMeasurementTime >= TICK_TIME) {
    ...
    // 3b. Bus Maintenance: Periodic re-discovery of I2C modules
    if (currentTime - lastBusDiscoveryTime >= BUS_DISCOVERY_INTERVAL) {
      lastBusDiscoveryTime = currentTime;

      /* Summary of function: Periodically scans the I2C bus to find new or re-connected modules without rebooting. */
      uint8_t found = ModuleEngine.scanAndDiscover();

      if (found > 0) {
        Serial.printf("[SCAN] Periodic scan done. Found %u module(s).\n", found);
      }
    }
  }


  // ── 4. Screen & Visualisation ───────────────────────────────────
  // Redraws the OLED display at the interval defined in the screen design.
  // Widget values are resolved live via BindingEngine (sensors, expressions,
  // remote nodes) — no direct coupling to other managers.
  // If saveScreenDesign() was called via the web interface since last loop,
  // hasChanges() will be true and the new layout is persisted to EEPROM.
#if SCREEN_MODULE
  displayManager.updateScreen();
  if (displayManager.hasChanges()) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (currentTime - lastPublishTime >= config.operation.publishInterval) {
      ...
      for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
        ...
        // Read the latest sampled value from the sensor manager cache.
        ...
        DEBUG_PRINTF(DEBUG_PUBLISH, "[Publish] Sensor %d delta: %.2f\n", i, delta);

        // Milliseconds since this value was last published.
